...
class PdfVisualizarConsolidacaoCompleta extends AbstractPdfRegistroPreco
{
    ...
    private function listarItensIntencao()
    {
        ...
        //    kim cr#228067
        if(empty($_SESSION['telaAppView'])){
            $this->Cell(67, 10, 'DESCRIÇÃO MATERIAL/SERVIÇO', 1, 0, 'C', 1);
            $this->Cell(68, 10, 'DESCRIÇÃO DETALHADA', 1, 0, 'C', 1);
        }else{
            $this->Cell(97, 10, 'DESCRIÇÃO MATERIAL/SERVIÇO', 1, 0, 'C', 1);
            $this->Cell(98, 10, 'DESCRIÇÃO DETALHADA', 1, 0, 'C', 1);
        }
        ...
        if(empty($_SESSION['telaAppView'])){
            ...
            $x = $this->GetX() + 97;
            $y = $this->GetY();
            $this->MultiCell(97, $alturaDesc, removeSimbolos($itemIntencao->descricao), 1, 'L', 0);
         }
            $this->SetXY($x, $y);
         //    kim cr#228067
         if(empty($_SESSION['telaAppView'])){
            ...
            $x = $this->GetX() + 98;
            $y = $this->GetY();
            $this->MultiCell(98, $alturaDescDetalhada, removeSimbolos($itemIntencao->descricaoDetalhada), 1, 'L', 0);
         }
            $this->SetXY($x, $y);
