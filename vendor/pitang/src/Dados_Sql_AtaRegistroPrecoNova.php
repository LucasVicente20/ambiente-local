...
# Alterado: Pitang Agile TI - Caio Coutinho
# Data: 17/09/2018
# Objetivo: Tarefa Redmine 205789
#-----------------------------------------------------------------
...
class Dados_Sql_AtaRegistroPrecoNova
{
     ...
     * @return string
     */
    public static function sqlAtaLicitacaoList($processo, $orgao, $ano, $situacao = 'A')
    {
        $ano = filter_var($ano, FILTER_SANITIZE_NUMBER_INT);
        ...
        if($situacao != 'A') {
            $sql .= " AND arpi.farpinsitu = '" .$situacao . "'";
        }
        
        $sql .= "  ORDER BY
                arpi.aarpinanon DESC,
                arpi.carpincodn ASC,
                arpi.carpnosequ";

        return sprintf($sql, $valores[0], $valores[1], $valores[2], $valores[3], $valores[4], $situacao);
    }
