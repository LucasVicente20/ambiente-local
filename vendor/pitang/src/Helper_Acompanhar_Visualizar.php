 ...
 * Data:     06/10/2018
 * Objetivo: Tarefa Redmine 206453
 * -----------------------------------------------------------------------------
 * Alterado: Lucas Baracho
 * Data:     04/06/2019
...
class Helper_Acompanhar_Visualizar
{
    ...
    public static function sqlQuantidadeConsolidada($cintrpsequ, $cintrpsano, $citirpsequ) {
        $sql = 'SELECT SUM (IRIRP.AIRIRPQTPR) AS CONSOLIDADO , CITIRPSEQU
                FROM SFPC.TBITEMRESPOSTAINTENCAORP IRIRP
                WHERE CINTRPSEQU = %d 
                AND CINTRPSANO = %d
                AND CITIRPSEQU = %d
                GROUP BY CINTRPSANO, CITIRPSEQU';
        return sprintf($sql, $cintrpsequ, $cintrpsano, $citirpsequ );
    }
    ...
    public static function sqlSelectItemIntencao($sequencialIntencao, $anoIntencao, $centroCusto)
    {
        ...
        $sql = " SELECT irirp.cintrpsequ, irirp.cintrpsano, irirp.citirpsequ, irirp.corglicodi, irirp.airirpqtpr, iirp.cmatepsequ, 
                        m.ematepdesc, iirp.cservpsequ, s.eservpdesc, iirp.EITIRPDESCSE, iirp.AITIRPORDE, iirp.VITIRPVUES, m.FMATEPGENE
                FROM
                    sfpc.tbitemrespostaintencaorp irirp
                    INNER JOIN sfpc.tbitemintencaoregistropreco iirp ON iirp.cintrpsequ = irirp.cintrpsequ
                    ...
                    INNER JOIN sfpc.tbintencaoregistropreco irp ON iirp.cintrpsequ = irp.cintrpsequ
                ...
                WHERE irirp.cintrpsequ = $sequencialIntencao
                    AND irirp.cintrpsano = $anoIntencao                    
                    AND irirp.airirpqtpr > 0
                ORDER BY
                    iirp.aitirporde "; #AND irirp.corglicodi = $orgaoUsuarioLogado
    ...
    public static function sqlSelectIntencao($sequencialIntencao, $anoIntencao, $orgao = null) {
        $sql = "SELECT  DISTINCT    A.CINTRPSEQU, A.CINTRPSANO, A.TINTRPDLIM, A.XINTRPOBJE,
                                    ...
                                    A.TINTRPULAT, OL.EORGLIDESC  ";

            if (!is_null($orgao)) {
                $sql .= ", UP.EUSUPORESP, RI.TRINRPULAT, RI.XRINRPJUST ";
            }
        ...
        $sql .= "  WHERE   A.CINTRPSEQU = $sequencialIntencao
