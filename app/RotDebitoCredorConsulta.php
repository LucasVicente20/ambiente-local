...
if ($TipoDoc == 1) {
    ...
    for ($c=0;$c<$row;$c++) {
        $SequCont = $SequencialDebito[$c];
                # Conectando ao SFCM.TBMERCANTIL para verificar se existe Inscrição Mercantil #
    ...
    $Url = "$NomePrograma?ProgramaSelecao=$ProgramaSelecao&Irregularidade=$Irregularidade&Destino=$Destino&Origem=A&Sequencial=$Sequencial&Situacao=$Situacao&CPF_CNPJ=$CPF_CNPJ&Botao=$Botao&TipoDoc=$TipoDoc";
