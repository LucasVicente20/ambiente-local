...
//função pra pegar a razão social do fornecedor
...
$filtroPesquisa = array(
    ...
    'LicitacaoAno' => $_SESSION['Export']['LicitacaoAno'],
...
// var_dump($filtroPesquisa['dataInicio']);die;
...
if ($Mens == 0) {
    ...
    $novaSql .= " AND A.XLICPOOBJE LIKE '%COVID%'";
    ...
    if ($filtroPesquisa['dataInicio'] != '') {
        ...
        // Ano
        if ($filtroPesquisa['LicitacaoAno'] != '') {
            $LicitacaoAno = $filtroPesquisa['LicitacaoAno'];
...
for( $i2=0;$i2<$numeroDeCnpj;$i2++ ){
    ...
    if(empty($concatenaRazaoCnpj)){
        ...
        $razaoCpf =  $exibir.' ___ '.$validaRazaoSocial;
...
$nomeArquivo = 'pcr_portal_compras_todas_licitacoes_covid_imunizacao';
