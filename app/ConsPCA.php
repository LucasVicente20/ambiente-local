...
function proccessPrincipal()
{
    ...
    $jsonString = file_get_contents($path.'publicacao.json');
    // Converte a string JSON para um objeto PHP
    $dadosPub = json_decode($jsonString);
    
    $tpl = new TemplateAppPadrao("templates/ConsPCA.html", "ConsPCA");
    //Monta os TPL
    $tpl->CAMINHO_PCA   =   'CadVisualizarPCA.php?doc='.$dadosPub->PCA->nomeDoc;
    $tpl->CAMINHO_PCACC =   'CadVisualizarPCA.php?doc='.$dadosPub->PCACC->nomeDoc;
    $tpl->TITULO_PCA    =   $dadosPub->PCA->titulo;
    $tpl->TITULO_PCACC  =   $dadosPub->PCACC->titulo;
    $tpl->block("BLOCO_PUBLICACOES");
    $tpl->show();
}
/**
