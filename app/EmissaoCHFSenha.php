...
$tpl = new TemplateAppPadrao("templates/EmissaoCHFSenha.html", "EmissaoCHFSenha");
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($cpfcnpj === true) {
        $tpl->CPF_CNPJ = removeSimbolos($CPF_CNPJ);
        ...
        if ($Codigo == "") {
            ...
            if (strtoupper2($Codigo) != $_SESSION['_Combinacao_']) {
                ...
                $Tipo = 2;
                $Mod = 1;
                $Mensagem .= "<a href=\"javascript:document.EmissaoCHFSenha.Codigo.focus();\" class=\"titulo2\">C&oacute;digo V&aacute;lido</a>";
            }
        }
    } else {
        $Codigo = "";
    ...
    if ($Mens == 0) {
        ...
        if (strlen(removeSimbolos($CPF_CNPJ)) == 11) {
            $sqlfor .= "AFORCRCCPF = '".removeSimbolos($CPF_CNPJ)."'";
        } elseif (strlen(removeSimbolos($CPF_CNPJ)) == 14) {
...
/**
 * BEGIN adicionado por Pitang Agile TI
 */
