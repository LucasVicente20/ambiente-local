...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($Botao == 'carregaProcesso') {
        ...
        Pitang_GetUrl::run('ConsTodasLicitacoesCovidDetalhes.php', true);
    }
    ...
    if($Pesquisa == 1){
        ...
        if( !$dataIniValida and !$dataFimValida ){
            ...
            Pitang_GetUrl::run('ConsTodasLicitacoesCovid.php', true);
        }
        ...
        if( !$dataIniValida || !$dataFimValida ){
            ...
            header('Location: ConsTodasLicitacoesCovid.php');
            exit();
        }
    }else{
        $dataIni = '';
...
if(empty($LicitacaoAno)){
    $LicitacaoAno = $_SESSION['LicitacaoAno'];
}
$tpl = new TemplateAppPadrao('templates/ConsTodasLicitacoesCovidResultado.html', 'ConsTodasLicitacoesCovidResultado');
...
if ($Mens == 0) {
    ...
    // Objeto
    if ($Objeto != '') {    
        $novaSql .= " AND (A.XLICPOOBJE LIKE '%COVID $Objeto%' OR A.XLICPOOBJE LIKE '%COVID ".RetiraAcentos($Objeto)."%' OR A.XLICPOOBJE LIKE '%IMUNIZAÇÃO $Objeto%' OR A.XLICPOOBJE LIKE '%IMUNIZAÇÃO ".RetiraAcentos($Objeto)."%' OR A.XLICPOOBJE LIKE '%IMUNIZACAO $Objeto%' OR A.XLICPOOBJE LIKE '%IMUNIZACAO ".RetiraAcentos($Objeto)."%' OR A.XLICPOOBJE LIKE '%IMUNIZAÇAO $Objeto%' OR A.XLICPOOBJE LIKE '%IMUNIZAÇAO ".RetiraAcentos($Objeto)."%' OR A.XLICPOOBJE LIKE '%IMUNIZACÃO $Objeto%' OR A.XLICPOOBJE LIKE '%IMUNIZACÃO ".RetiraAcentos($Objeto)."%')";
    }else{
        $novaSql .= " AND (A.XLICPOOBJE LIKE '%COVID%' OR A.XLICPOOBJE LIKE '%IMUNIZAÇÃO%' OR A.XLICPOOBJE LIKE '%IMUNIZACAO%' OR A.XLICPOOBJE LIKE '%IMUNIZAÇAO%' OR A.XLICPOOBJE LIKE '%IMUNIZACÃO%')";
    }
    ...
    $novaSql .= " AND ((D.CFASESCODI IN ($strIdTodasSituacoes)) OR (D.CFASESCODI = 24))";
