...
/**
 * 
 * Nome: AtualizaDocumentosInscrito.php
...
//var_dump($_SESSION['Arquivos_Upload']);
...
$sql = "
SELECT
    aprefosequ, aprefoccgc, aprefoccpf, aprefoiden, nprefoorgu, 
    cprefscodi, nprefosenh, eprefomoti, npreforazs, nprefofant, 
    cceppocodi, eprefologr, aprefonume, eprefocomp, eprefobair, 
    nprefocida, cprefoesta, aprefocddd, aprefotels, aprefonfax, 
    nprefomail, aprefocpfc, nprefocont, nprefocarg, aprefodddc, 
    aprefotelc, apreforegj, dpreforegj, aprefoines, aprefoinme, 
    aprefoinsm, vprefocaps, vprefocapi, vprefopatl, vprefoinlc, 
    vprefoinlg, dprefoultb, dprefocnfc, nprefoentp, aprefoentr, 
    dprefovige, aprefoentt, dprefogera, FORN.cgrempcodi, FORN.cusupocodi, 
    aprefonten, dprefoexps, tprefoulat, cceloccodi, FORN.ccomlicodi, 
    dprefoanal, fprefomepp, vprefoindi, vprefoinso, nprefomai2, 
    fprefotipo, dprefocont
FROM SFPC.TBPREFORNECEDOR FORN
			...
			if (count($_SESSION['Arquivos_Upload']) != 0) {
				for ($i=0; $i< count($_SESSION['Arquivos_Upload']); $i++) {
					...
					if($arquivo['situacao'][$i] == 'novo'){
						...
						VALUES(".$seqDocumento.", ".$Sequencial.", NULL,".date('Y').", ".$arquivo['tipoCod'][$i].", '".$arquivo['nome'][$i]."', decode('".$anexo."','hex'), 'S', now(), 'A', ".$_SESSION['_cusupocodi_'].", now());
						";

						//print_r($sqlAnexo);
                        
                        $resultAnexo = $db->query($sqlAnexo);
                        //var_dump($resultAnexo);
                        //die();
                ...
                if ( !(PEAR::isError($resultFinal)) ) {
                    header('Location: AtualizaDocumentosInscrito.php?Sequencial='.$Sequencial.'&atualizacao=1');
                }
