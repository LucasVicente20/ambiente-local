...
#-------------------------------------------------------------------------
# Portal da DGCO
# Programa: CadCHFSelecionar.php
# Autor:    Roberta Costa
# Data:     21/09/04
# Objetivo: Programa que Selecionar o Fornecedor para Alterar data do CHF
...
AddMenuAcesso( '/fornecedores/CadCHFAlterar.php' );
	  ...
	  exit;
		...
		if( $Argumento != "" ){
				if( ($ItemPesquisa == "CNPJ") and (!SoNumeros($Argumento)) ){
			    	$Mens 		 = 1;$Tipo = 2;
						$Mensagem .= "<a href=\"javascript:document.CadCHFSelecionar.Argumento.focus();\" class=\"titulo2\">CNPJ Válido</a>";
				}else{
						if( ($ItemPesquisa == "CPF") and (!SoNumeros($Argumento)) ){
					    	$Mens 		 = 1;$Tipo = 2;
								$Mensagem .= "<a href=\"javascript:document.CadCHFSelecionar.Argumento.focus();\" class=\"titulo2\">CPF Válido</a>";
						}
				}
		}else{
		  	$Mens 		 = 1;
		  	$Tipo      = 2;
				$Mensagem .= "<a href=\"javascript:document.CadCHFSelecionar.Argumento.focus();\" class=\"titulo2\">Argumento da Pesquisa</a>";
		}
}
?>
<html>
...
function enviar(valor){
	document.CadCHFSelecionar.Botao.value=valor;
	document.CadCHFSelecionar.submit();
}
<?php MenuAcesso(); ?>
//-->
</script>
...
<form action="CadCHFSelecionar.php" method="post" name="CadCHFSelecionar">
<br><br><br><br><br>
<table cellpadding="3" border="0" summary="">
  ...
  <tr>
    ...
    <td align="left" class="textonormal" colspan="2">
      ...
      <a href="../index.php"><font color="#000000">Página Principal</font></a> > Fornecedores > CHF > Alteração da Data do CHF
    </td>
  </tr>
  <!-- Fim do Caminho-->
	...
	<tr>
      ...
      <table border="0" cellspacing="0" cellpadding="3" summary="">
        <tr>
	      	<td class="textonormal">
	        	<table border="1" cellpadding="3" cellspacing="0"  bordercolor="#75ADE6" bgcolor="#ffffff" class="textonormal" width="100%" summary="">
	          	<tr>
	  	      	...
	  	      	<tr>
	    	      	<td class="textonormal" colspan="4">
	      	    		<p align="justify">
	        	    		...
	        	    		alterar a data de validade do CHF. Só irão ser selecionados os fornecedores
	        	    		que possuem data do CHF cadastrada.<br><br>
      	      ...
      	      <tr>
		        	...
		        	<?php
							if( $Botao == "Pesquisar" and $Mens == 0 ){
									...
									$sql .= "       A.NFORCRRAZS, B.DFORCHVALI, A.APREFOSEQU, C.DPREFOGERA ";
									$sql .= "  FROM SFPC.TBFORNECEDORCREDENCIADO A ";
									$sql .= "  LEFT JOIN  SFPC.TBFORNECEDORCHF AS B ON B.AFORCRSEQU = A.AFORCRSEQU";
									$sql .= "  LEFT JOIN  SFPC.TBPREFORNECEDOR AS C ON C.APREFOSEQU = A.APREFOSEQU";
									//$sql .= " WHERE A.AFORCRSEQU = B.AFORCRSEQU AND B.DFORCHVALI < '".date("Y-m-d")."' AND ";
									$sql .= " WHERE A.AFORCRSEQU = B.AFORCRSEQU AND B.DFORCHVALI IS NOT NULL  AND ";
									if( $Palavra == 1 ){
									...
									if( PEAR::isError($result) ){
							    		ExibeErroBD("$ErroPrograma\nLinha: ".__LINE__."\nSql: $sql");
											...
											if( $Qtd > 0 ){
													...
													echo "			<td class=\"titulo3\" bgcolor=\"#F7F7F7\" align=\"center\">DATA VALIDADE</td>\n";
													echo "		</tr>\n";
													$Sequencial	= "";
													while( $Linha	= $result->fetchRow() ){
															...
															$DataInscSicref = ($Linha[7] != "") ? substr($Linha[7], 8, 2).'/'.substr($Linha[7], 5, 2).'/'.substr($Linha[7], 0, 4).' '.substr($Linha[7], 11, 9) : ' - ';
															echo "	<tr>\n";
															if( $CNPJ <> 0 ){
							             				$CNPJForm = $CNPJ;
							             				...
							             				$CPFForm = (substr($CPFForm,0,3).".".substr($CPFForm,3,3).".".substr($CPFForm,6,3)."-".substr($CPFForm,9,2));
															...
															if( ($Situacao == 2) or ($Situacao == 5) ){
																	...
																	$Url = "CadCHFAlterar.php?Sequencial=$Sequencial";
