...
#-------------------------------------------------------------------------
# Portal da DGCO
# Programa: CadCertidaoSelecionar.php
# Autor:    Roberta Costa
# Data:     16/09/04
...
AddMenuAcesso( '/fornecedores/CadCertidaoAlterar.php' );
...
if( $Botao == "Pesquisar" ){
		...
		if( $Argumento != "" ){
				if( ($ItemPesquisa == "CNPJ") and (!SoNumeros($Argumento)) ){
			    	$Mens 		 = 1;$Tipo = 2;
						$Mensagem .= "<a href=\"javascript:document.CadCertidaoSelecionar.Argumento.focus();\" class=\"titulo2\">CNPJ Válido</a>";
				}else{
						if( ($ItemPesquisa == "CPF") and (!SoNumeros($Argumento)) ){
					    	$Mens 		 = 1;$Tipo = 2;
								$Mensagem .= "<a href=\"javascript:document.CadCertidaoSelecionar.Argumento.focus();\" class=\"titulo2\">CPF Válido</a>";
						}
				}
		}else{
		  	$Mens 		 = 1;
		  	$Tipo      = 2;
				$Mensagem .= "<a href=\"javascript:document.CadCertidaoSelecionar.Argumento.focus();\" class=\"titulo2\">Argumento da Pesquisa</a>";
		}
}
?>
...
function enviar(valor){
	document.CadCertidaoSelecionar.Botao.value=valor;
	document.CadCertidaoSelecionar.submit();
}
<?php MenuAcesso(); ?>
//-->
</script>
...
<form action="CadCertidaoSelecionar.php" method="post" name="CadCertidaoSelecionar">
<br><br><br><br><br>
  ...
  <tr>
    ...
    <td align="left" class="textonormal" colspan="2">
      <font class="titulo2">|</font>
 	    <a href="../index.php"><font color="#000000">Página Principal</font></a> > Fornecedores > Atualização de Certidão
    </td>
  </tr>
  <!-- Fim do Caminho-->
	    	      	...
	    	      	<td class="textonormal" colspan="4">
	      	    		<p align="justify">
	        	    		...
	        	    		clique no botão "Pesquisar". Depois, clique no fornecedor desejado para
										atualizar as datas das certidões vencidas.<br>
	        	    		Para limpar a pesquisa, clique no botão "Limpar".<br><br>
	        	    		Só aparecerão os fornecedores que possuem as certidões vencidas.
	          	   	</p>
	          		</td>
	          	</tr>
		        	<tr>
      	      ...
      	      <tr>
		        	...
		        	<?php
							if( $Botao == "Pesquisar" and $Mens == 0 ){
									...
									$db	  = Conexao();
									$sql  = "SELECT DISTINCT A.DFORCRGERA, A.AFORCRSEQU, A.AFORCRCCGC, A.AFORCRCCPF, A.NFORCRRAZS "; //, B.DFORSISITU  ";
									$sql .= "  FROM SFPC.TBFORNECEDORCREDENCIADO A, SFPC.TBFORNECEDORCERTIDAO B  ";
									$sql .= " WHERE A.AFORCRSEQU = B.AFORCRSEQU AND B.DFORCEVALI < '".date("Y-m-d")."' AND ";
									if( $Palavra == 1 ){
													...
													while( $Linha	= $result->fetchRow() ){
							             				...
							             				$CPFForm = (substr($CPFForm,0,3).".".substr($CPFForm,3,3).".".substr($CPFForm,6,3)."-".substr($CPFForm,9,2));
															...
															if( ($Situacao == 2) or ($Situacao == 5) ){
																	...
																	$_SESSION['Critica'] = "";
																	$Url = "CadCertidaoAlterar.php?Sequencial=$Sequencial";
