...
# Alterado: Eliakim Ramos
# Data:     13/01/2023
# Objetivo: A parte de socio não estava aparecendo
#-------------------------------------------------------------------------
...
# Tabela Inicial de Todos os Formulários #
	...
	$_SESSION['Motivo']					= strtoupper(trim($_POST['Motivo']));
...
if ($Origem == "A") {
	...
	$_SESSION['Identidade']			= strtoupper(trim($_POST['Identidade']));
	$_SESSION['OrgaoUF']				= strtoupper(trim($_POST['OrgaoUF']));
	$_SESSION['RazaoSocial']		= strtoupper(trim($_POST['RazaoSocial']));
	$_SESSION['NomeFantasia'] 	= strtoupper(trim($_POST['NomeFantasia']));
	$_SESSION['CEP']						= $_POST['CEP'];
	$_SESSION['CEPAntes']				= $_POST['CEPAntes'];
	$_SESSION['Logradouro']			= strtoupper(trim($_POST['Logradouro']));
	$_SESSION['Numero']					=	$_POST['Numero'];
	$_SESSION['Complemento']		= strtoupper(trim($_POST['Complemento']));
	$_SESSION['Bairro']					= strtoupper(trim($_POST['Bairro']));
	$_SESSION['Cidade']					= strtoupper(trim($_POST['Cidade']));
	...
	$_SESSION['NomeContato']		= strtoupper(trim($_POST['NomeContato']));
	$_SESSION['CPFContato']			= $_POST['CPFContato'];
	$_SESSION['CargoContato'] 	= strtoupper(trim($_POST['CargoContato']));
...
# Variáveis do Formulário C #
	if ($Origem == "C") {
		...
		$_SESSION['Banco1']							 = strtoupper(trim($_POST['Banco1']));
		$_SESSION['Agencia1']						 = strtoupper(trim($_POST['Agencia1']));
		$_SESSION['ContaCorrente1']			 = strtoupper(trim($_POST['ContaCorrente1']));
		$_SESSION['Banco2']							 = strtoupper(trim($_POST['Banco2']));
		$_SESSION['Agencia2']						 = strtoupper(trim($_POST['Agencia2']));
		$_SESSION['ContaCorrente2']			 = strtoupper(trim($_POST['ContaCorrente2']));
...
# Variáveis do Formulário D #
	if ($Origem == "D") {
		...
		$_SESSION['NomeEntidade']		 = strtoupper(trim($_POST['NomeEntidade']));
		$_SESSION['DataVigencia']		 = $_POST['DataVigencia'];
		$_SESSION['RegistroTecnico'] = $_POST['RegistroTecnico'];
		$_SESSION['AutorizaNome']		 = strtoupper(trim($_POST['AutorizaNome']));
...
# Busca os Dados da Tabela de Situação de acordo com o sequencial do Fornecedor #
	...
	if (PEAR::isError($result)) {
		...
		$_SESSION['Motivo']						= strtoupper($Linha[2]);
...
# Variáveis do Formulário A #
		...
		if ($_SESSION['NomeFantasia'] == "") {
				...
				$NomeFantasia = stripslashes($NomeFantasia);
			}
		if ($_SESSION['DDD'] == "") {
				$DDD = "NULL";
		} else {
...
# Atualiza a tabela de Fornecedor #
		$sql = " UPDATE SFPC.TBFORNECEDORCREDENCIADO
							  SET AFORCRIDEN = $Identidade, NFORCRORGU = $OrgaoUF,
										NFORCRRAZS = '" . stripslashes($_SESSION['RazaoSocial']) . "',
			...
			if (count($_SESSION['CertidaoObrigatoria']) != 0) {
				...
				if (PEAR::isError($result)) {
					...
					for ($i = 0; $i < count($_SESSION['CertidaoObrigatoria']); $i++) {
						if ((!is_null($_SESSION['CertidaoObrigatoria'][$i])) && ($_SESSION['DataCertidaoOb'][$i] != "")) {
...
# Excluindo e  Inserindo Certidões COMPLEMENTARES #
			if (count($_SESSION['CertidaoComplementar']) == 0) {
				...
				for ($i = 0; $i < count($_SESSION['CertidaoComplementar']); $i++) {
					...
					if ((!is_null($_SESSION['CertidaoComplementar'][$i])) && ($_SESSION['DataCertidaoComp'][$i] != "")) {
						$sql = "INSERT INTO SFPC.TBFORNECEDORCERTIDAO ( ";
						$sql .= "AFORCRSEQU, CTIPCECODI, DFORCEVALI, TFORCEULAT ";
						$sql .= ") VALUES ( ";
			...
			if (count($_SESSION['Arquivos_Upload']) != 0) {
				for ($i=0; $i< count($_SESSION['Arquivos_Upload']); $i++) {
					...
					if($arquivo['situacao'][$i] == 'novo'){
						...
						$arquivo['nome'][$i] = tratarNomeArquivo(RetiraAcentosVirgula($arquivo['nome'][$i]));
...
# Verifica se a data de CHF já esta cadastrada #
				...
				if (PEAR::isError($res)) {
						...
						$Rows = $res->numRows();
					if ($Rows == 0) {
						$_SESSION['DataValidadeCHF'] = date_transform($_SESSION['DataGeracaoCHF']);
						$_SESSION['DataValidadeCHF'] = date('Y-m-d', strtotime($_SESSION['DataValidadeCHF'] . ' + 1 years'));
						//var_dump($_SESSION['DataValidadeCHF']);exit;
...
# Inserindo Situação Diferente de Cadastrado #
				...
				$sql .= ") VALUES ( ";
				$sql .= "" . $_SESSION['Sequencial'] . ", " . $_SESSION['_cgrempcodi_'] . ", " . $_SESSION['_cusupocodi_'] . ", '" . date_transform($_SESSION['DataGeracaoCHF']) . "', '" . $_SESSION['DataValidadeCHF'] . "', ";
					...
					if ($_SESSION['Mens'] == 0 and $_SESSION['Botao'] != "Preencher") {
						...
						if ($_SESSION['DataRegistro'] == "") {
							...
							if ($MensErro != "") {
								...
								if (date_transform($_SESSION['DataRegistro']) > date("Y-m-d")) {
									if ($_SESSION['Mens'] == 1) {
										$_SESSION['Mensagem'] .= ", ";
									}
									$_SESSION['Mens'] = 1;
...
# Mensagens para Irregularidade e Certidões Fora do Prazo de Validade #
	//if ($_SESSION['Mens'] == 0) {
		if (($_SESSION['TipoHabilitacao'] == 'L') && ($_SESSION['Situacao'] == 1)) { //compra direta e não-'CADASTRADO' pode possuir certidoes inválidas
			...
			if ($_SESSION['DataValidadeCHF'] != "" and (date_transform($_SESSION['DataValidadeCHF']) < date("Y-m-d")) and ($_SESSION['Situacao'] == 1) and ($_SESSION['Cadastrado'] == "HABILITADO")     ) {
				$_SESSION['Cadastrado'] = "INABILITADO";
		...
		if (is_null($_SESSION['DataCertidaoOb']) or !is_array($_SESSION['DataCertidaoOb'])) {
			...
			if (PEAR::isError($res)) {
				...
				for ($i = 0; $i < $Rows; $i++) {
					...
					if (date_transform($_SESSION['DataCertidaoOb'][$i]) < date("Y-m-d")  ) {
						$Validade = "titulo1";
					} else {
			...
			if (PEAR::isError($res)) {
				...
				if ($Rows != 0) {
					...
					for ($i = 0; $i < $Rows; $i++) {
						...
						if (date_transform($_SESSION['DataCertidaoComp'][$i]) < date("Y-m-d")    ) {
							$Validade = "titulo1";
						} else {
							$Validade = "textonormal";
						}
...
?>
					              				<input type="button" class="botao" value="Incluir Grupos" onclick="javascript:AbreJanela('<?php echo $Url;?>',750,370);">
	...
	if ($_SESSION['Botao'] == "Limpar") {
		...
		if ($_POST['tipoDoc'] == '0' ) {
			...
			if ($_FILES['Documentacao']['tmp_name']) {
				$_FILES['Documentacao']['name'] = tratarNomeArquivo(RetiraAcentosVirgula($_FILES['Documentacao']['name']));
				...
				if ($_SESSION['Mens'] == '') {
					if (! ($_SESSION['Arquivos_Upload']['conteudo'][] = file_get_contents($_FILES['Documentacao']['tmp_name']))) {
						...
						$_SESSION['Arquivos_Upload']['observacao'][] = strtoupper($_POST['obsDocumento']); 
																	...
																	<tr>
																		...
																		<select name="tipoDoc" id="tipoDoc" class="tamanho_campo textonormal" onchange="preencheTipoDocDesc()">
																			...
																			<?php
																				...
																				if (PEAR::isError($res)) {
																					$ErroPrograma = "CadGestãoFornecedor";
																					ExibeErroBD("$ErroPrograma\nLinha: ".__LINE__."\nSql: $sql");
																				} else {
																				...
																				<td class='textonormal' bgcolor="#ffffff">
																					<?php 
																						if($_SESSION['Arquivos_Upload']['situacao'][$Dcont]=='existente'){
																							...
																							echo 'PCR - '.$_SESSION['Arquivos_Upload']['usuarioUltAlt'][0];
																						}
	...
	if ($_SESSION['Botao'] == "TipoSituacao") {
		...
		if ($_SESSION['Situacao'] != 1) {
			...
			elseif ($_SESSION['DataExpiracao'] != "") {
				...
				if ($MensErro != "") {
					...
					if (date_transform(($_SESSION['DataExpiracao'])) <= date_transform($_SESSION['DataSituacao'])) {
						if ($_SESSION['Mens'] == 1) {
							$_SESSION['Mensagem'] .= ", ";
						}
						$_SESSION['Mens'] = 1;
		...
		elseif ($_SESSION['DataGeracaoCHF'] != "") {
			...
			if ($MensErro != "") {
				...
				if (date_transform($_SESSION['DataGeracaoCHF']) > date("Y-m-d")) {
					if ($_SESSION['Mens'] == 1) {
						$_SESSION['Mensagem'] .= ", ";
					}
					$_SESSION['Mens'] = 1;
