 ...
 * Data:     23/07/2021
 * Objetivo: Tarefa Redmine  #249885
 * -----------------------------------------------------------------------
 * Autor:    João Madson
 * Data:     06/04/2021 
 * Objetivo: CR #245672
...
function EmailErroSistema($assunto, $mensagem, $crash = true) {
    ...
    if ($crash) {
							...
							<tr>
								<td bgcolor='#efeeee'>
									<dfont face='verdana,verdana,sans-serif' size='-1'>
										...
										Tente novamente mais tarde e, se o problema persistir, entre em contato com a Gerência Geral de Licitação e Compras, telefone 3355-8790, ou através do e-mail portalcompras@recife.pe.gov.br.
									</font>
								</td>
							</tr>
						</table>
        ...
        $usuario = $_SESSION['_eusupologi_'];

        $sql = "select ausupofone, eusupomail FROM sfpc.tbusuarioportal where eusupologi = '$usuario' ";
