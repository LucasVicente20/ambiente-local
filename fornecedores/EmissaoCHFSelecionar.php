...
#-------------------------------------------------------------------------
# Portal da DGCO
# Programa: EmissaoCHFSelecionar.php
# Autor:    Roberta Costa
# Data:     21/09/04
# Objetivo: Programa que Seleciona Fornecedor para Emissão do CHF
	  ...
	  exit;
		...
		if( $Argumento != "" ){
				if( ($ItemPesquisa == "CNPJ") and (!SoNumeros($Argumento)) ){
			    	$Mens 		 = 1;
			    	$Tipo      = 2;
						$Mensagem .= "<a href=\"javascript:document.EmissaoCHFSelecionar.Argumento.focus();\" class=\"titulo2\">CNPJ Válido</a>";
				}else{
						if( ($ItemPesquisa == "CPF") and (!SoNumeros($Argumento)) ){
					    	$Mens 		 = 1;
					    	$Tipo      = 2;
								$Mensagem .= "<a href=\"javascript:document.EmissaoCHFSelecionar.Argumento.focus();\" class=\"titulo2\">CPF Válido</a>";
						}
				}
		}else{
		  	$Mens 		 = 1;
		  	$Tipo      = 2;
				$Mensagem .= "<a href=\"javascript:document.EmissaoCHFSelecionar.Argumento.focus();\" class=\"titulo2\">Argumento da Pesquisa</a>";
		}
}
?>
<html>
...
function enviar(valor){
	document.EmissaoCHFSelecionar.Botao.value=valor;
	document.EmissaoCHFSelecionar.submit();
}
<?php MenuAcesso(); ?>
//-->
</script>
...
<form action="EmissaoCHFSelecionar.php" method="post" name="EmissaoCHFSelecionar">
<br><br><br><br><br>
	    	      	...
	    	      	<td class="textonormal" colspan="4">
	      	    		<p align="justify">
	        	    		Selecione o item de pesquisa desejado, preencha o argumento da pesquisa e  clique no botão "Pesquisar".
										Depois, clique no fornecedor desejado para emitir o Certificado de Habilitação de Firmas (CHF).<br>
...
function Resultado($Qtd,$result,$TipoForn){
	...
	if( $Qtd > 0 ){
			echo "		<tr>\n";
			if( $ItemPesquisa == "CNPJ" ){ 
					echo "	<td class=\"titulo3\" bgcolor=\"#DCEDF7\" width=\"15%\">CNPJ</td>\n";
			}elseif( $ItemPesquisa == "CPF" ){
					echo "	<td class=\"titulo3\" bgcolor=\"#F7F7F7\" width=\"15%\">CPF</td>\n";
			}else{
					echo "	<td class=\"titulo3\" bgcolor=\"#F7F7F7\" width=\"15%\">CNPJ/CPF</td>\n";
			}
			echo "			<td class=\"titulo3\" bgcolor=\"#F7F7F7\">RAZÃO/NOME</td>\n";
			echo "			<td class=\"titulo3\" bgcolor=\"#F7F7F7\" align=\"center\" width=\"14%\">DATA CADASTRAMENTO</td>\n";
			echo "		</tr>\n";
			$Sequencial	= "";
			while( $Linha	= $result->fetchRow() ){
					...
					$CPF			= $Linha[3];
					$Razao         	= $Linha[4];
					$DataInscricao 	= DataBarra($Linha[0]);
					...
					if( $CNPJ != 0 ){
							$Url = "EmissaoCHF.php?Sequencial=$Sequencial&CPF_CNPJ=$CNPJ&TipoDoc=1";
							if (!in_array($Url,$_SESSION['GetUrl'])){ $_SESSION['GetUrl'][] = $Url; }
							echo "	<td valign=\"top\" bgcolor=\"#F7F7F7\" class=\"textonormal\"><a href=\"$Url\"><font color=\"#000000\">$CNPJForm</font></td>\n";
					}else{
							$Url = "EmissaoCHF.php?Sequencial=$Sequencial&CPF_CNPJ=$CPF&TipoDoc=2";
							if (!in_array($Url,$_SESSION['GetUrl'])){ $_SESSION['GetUrl'][] = $Url; }
