...
$arrBase = getProcessosRelGerencialTramitacao($buscar, '');
...
<td colspan="4" class="titulo3" bgcolor="#D3D3D3" class="textonormal" align="center">COMISSÃO : '.$dados[0][16].'</td>
</tr>';
if(!empty($arrBase)) { 
			...
			foreach($arrBase as $value) {
				...
				$htmlCabecalhoItem .= '
				<tr>
					<td rowspan="2" class="titulo3" bgcolor="#D3D3D3" class="textonormal" align="center">PROCESSO</td>
					<td colspan="3" class="titulo3" bgcolor="#D3D3D3" class="textonormal" align="center">MÉDIA EM DIAS POR AÇÃO</td>
				</tr>
				<tr>';
				...
				foreach($arrAcoes as $objAcao) {
					...
					//var_dump($objAcao[1].' - '.$codModalidade.' - '.$value[1].' - '.$dados);
					//die();
				...
				$htmlItem .= '<tr><td align="center">'.str_pad($value[0], 4, "0", STR_PAD_LEFT)."/".$value[1].'</a></td>';
