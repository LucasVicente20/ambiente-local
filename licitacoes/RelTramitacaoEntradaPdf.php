...
#------------------------------------------------------------------------------------
# Portal da DGCO
# Programa: RelTramitacaoEntradaPdf.php
...
# Data:     10/06/2019
# Objetivo: Tarefa Redmine 218623
#------------------------------------------------------------------------------------
# Alterado: Lucas Vicente
# Data:     13/09/2022
# Objetivo: Tarefa Redmine 268380 
#------------------------------------------------------------------------------------
...
if (count($comissoesUsuario)){
	...
	////('agente'); OR (tram.cusupocodi = ".$_SESSION['_cusupocodi_'].")
	$sql .= " AND prot.cprotcsequ in (select tram.cprotcsequ from sfpc.tbtramitacaolicitacao tram
	join sfpc.tbtramitacaoacao ac on tram.ctacaosequ = ac.ctacaosequ 
	join sfpc.tbtramitacaoagente agen on tram.ctagensequ = agen.ctagensequ
...
/*AND (SELECT TL.cusupocodi
			FROM SFPC.TBTRAMITACAOLICITACAO TL
			where TL.cprotcsequ = prot.cprotcsequ
			order by  TL.ctramlsequ desc
			limit 1) = ".$_SESSION['_cusupocodi_']."*/
...
$sql .= " 
        ...
        // Data Entrada
        if($DataEntradaIni != "" or $DataEntradaFim != ""){
			$htmlPesquisa .= "<tr>
								<td width='15%' class='campo_pesquisado'>PERÍODO</td>
								<td>".$DataEntradaIni." A ".$DataEntradaFim."</td>
							</tr>";
			//SQL
			...
			if ( $DataEntradaFimFormatada == "--" ) {
				$DataEntradaFimFormatada = " now() ";
			} else if ($DataEntradaIniFormatada == "--" ) {
				$DataEntradaIniFormatada = " now() ";
			}
...
if( PEAR::isError($resitens) ){
		...
		if($rowsitens > 0){
			...
			$htmlCabecalhoItem .="<td align='center' class='campo_pesquisado' rowspan='2' width='7%'>NÚMERO/<BR>ANO<BR>PROTOCOLO</td>";
			...
			$htmlCabecalhoItem .="<td align='center' class='campo_pesquisado' rowspan='2' width='5%' border='1'>FASE ATUAL</td>";
			$htmlCabecalhoItem .="<td align='center' class='campo_pesquisado' rowspan='2' width='6%'>DATA/<BR>HORA<BR>ENTRADA</td>";
			...
			while($Linha = $resitens->fetchRow()){
				...
				if(!empty($Linha[8])){ 
					...
					// processo licitatorio e fase
				
					if(!empty($arrProcesso[0])) {
						$procLic = str_pad($arrProcesso[0], 4, "0", STR_PAD_LEFT)."/".$arrProcesso[1]." - ". $arrProcesso[5];
						$faseLic = $arrFase[1];
				...
				$dataEntrada = date('d/m/Y H:i:s',strtotime($Linha[19]));
				//Dados da Ação                                      
				$acao = $Linha[22];
				$origem = $Linha[23];
				...
				$htmlItem .= "<td align='center'>".$dataEntrada."</td>";
			...
			$html .= "</table>";			
				...
				$Url = "CadTramitacaoEntrada.php?retornoVazioPdf=1";
