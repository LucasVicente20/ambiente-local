...
# Alterado: Pitang Agile TI - Caio Coutinho
# Data:		28/12/2018
# Objetivo:	Tarefa Redmine 208658
#-------------------------------------------------------------------------
...
//madson | Função para validar a data da liberação do bloqueio
function validaDataLiberacaoBloqueio(){
    $db = conexao();
    // |Madson| O sql busca no caso de só haver um campo na tabela, checar com Rossana se é nescessário adaptar;
    $sql = "select tpargeilib, tpargeflib from sfpc.tbparametrosgerais where cpargetbid = 1";
    $res = $db->query($sql);
    if (PEAR::isError($res)) {
        $CodErroEmail  = $res->getCode();
        $DescErroEmail = $res->getMessage();
        ...
        while ($linha = $res->fetchRow()) {
            $datainicialVLB = $linha[0];
            $datafinalVLB = $linha[1];
        }
    }
   
    $DataIniLib = strtotime($datainicialVLB);
    $DataFimLib = strtotime($datafinalVLB);
    $hoje = strtotime(date('Y-m-d'));
    if($hoje >= $DataIniLib && $hoje <= $DataFimLib){
        return true;
    }else{
        return false;
    }
...
{   
    ...
    //Madson Invocação da nova função de validação da data de liberação
    $periodoVLibBloq = validaDataLiberacaoBloqueio();
    ...
    $valorTotalScc = 0;
    $bloqueiosValoresTotais = 2; // Adicionar 2 ao valor do bloqueio
    $arrayPos = -1;
    if (count($arrayReservas) == 0 or is_null($arrayReservas)) {
        ...
        foreach ($arrayReservas as $reserva) {
            ...
            if ($tipoReserva == TIPO_RESERVA_ORCAMENTARIA_DOTACAO) {
                ...
                // |MADSON| Conforme CR #208774 Foi adicionado o condicional a baixo para checagem da data de liberação da validação do bloqueio;
                if($periodoVLibBloq == false){   
    ...
    if ($tipoReserva == TIPO_RESERVA_ORCAMENTARIA_BLOQUEIO) {
        if ($bloqueiosValoresTotais<$valorTotalScc) {
            //Modificação 21/06/2017 - Solicitado pela pitang --- Início
			//erroUsuario($respostaComoMensagem, $nomeCampo, 'Valor total da Solicitação é maior que a soma de todos os bloqueios');
			erroUsuario($respostaComoMensagem, $nomeCampo, 'Valor total da Solicitação ('.$valorTotalScc.') é maior que a soma de todos os bloqueios  ('.$bloqueiosValoresTotais.')');
			//Modificação 21/06/2017 - Solicitado pela pitang --- Fim
        }
    }
}
...
function listaItensLicitacao($processo, $ano, $grupo, $comissao, $orgao)
{
    $db = $GLOBALS["db"];
    ...
    , ITEM.EITELPMARC , ITEM.EITELPMODE , ITEM.CMOTNLSEQU , ITEM.AFORCRSEQU
        , ITEM.EITELPDESCMAT, ITEM.EITELPDESCSE
    ...
    WHERE   ITEM.clicpoproc = $processo
        AND ITEM.alicpoanop = $ano
        AND ITEM.cgrempcodi = " . $grupo . "
        AND ITEM.ccomlicodi = $comissao
        AND ITEM.corglicodi = $orgao
    ORDER BY ITEM.CITELPNUML , ITEM.AITELPORDE , ITEM.CITELPSEQU";
...
function listaItensLicitacaoRascunho($processo, $ano, $grupo, $comissao, $orgao)
{
    $db = $GLOBALS["db"];

    $sql = "SELECT ITEM.CITELRSEQU , ITEM.CMATEPSEQU , ITEM.CSERVPSEQU , ITEM.AITELRORDE
    , ITEM.AITELRQTSO , ITEM.VITELRUNIT , ITEM.AITELRQTEX , ITEM.VITELRVEXE
    , MAT.EMATEPDESC, UNIDADE.EUNIDMSIGL , SERV.ESERVPDESC , ITEM.CITELRNUML  , ITEM.VITELRVLOG
    , ITEM.EITELRMARC , ITEM.EITELRMODE , ITEM.CMOTNLSEQU , ITEM.AFORCRSEQU
        , ITEM.EITELRDESCMAT, ITEM.EITELRDESCSE
    FROM SFPC.TBITEMLICITACAORASCUNHO ITEM ";
    $sql .= "LEFT JOIN SFPC.TBMATERIALPORTAL MAT ON (MAT.CMATEPSEQU = ITEM.CMATEPSEQU)
    LEFT JOIN  SFPC.TBSERVICOPORTAL SERV ON (SERV.CSERVPSEQU = ITEM.CSERVPSEQU)
    LEFT JOIN  SFPC.TBUNIDADEDEMEDIDA UNIDADE ON (MAT.CUNIDMCODI = UNIDADE.CUNIDMCODI)
    WHERE   ITEM.clicpoproc = $processo
        ...
        AND ITEM.ccomlicodi = $comissao
        AND ITEM.corglicodi = $orgao
    ORDER BY ITEM.CITELRNUML , ITEM.AITELRORDE , ITEM.CITELRSEQU";
...
function itensLicitacao($processo, $ano, $grupo, $comissao, $orgao)
{
    $db = $GLOBALS["db"];

    $sql = "SELECT * FROM SFPC.TBITEMLICITACAOPORTAL ITEM ";
...
function verificarRascunho($processo, $ano, $grupo, $comissao, $orgao) {
    $db = $GLOBALS["db"];
    $sql = "SELECT COUNT(*) 
            FROM SFPC.TBITEMLICITACAORASCUNHO
...
function verificarLicitacao($processo, $ano, $grupo, $comissao, $orgao) {
    $db = $GLOBALS["db"];
    $sql = "SELECT COUNT(*) 
            ...
            WHERE CLICPOPROC = $processo
                ...
                AND   CORGLICODI = $orgao
                AND   CITELPSEQU = 1
                AND   (VITELPVLOG <> 0 OR   CMOTNLSEQU <> 0)";
...
function listaMotivosNaoLogrado()
{
    $db = $GLOBALS["db"];
...
function ajustarColunasInsertRascunho($item)
{
    $itemRascunho = array();
    foreach ($item as $key => $value) {
        if(in_array($key, array(17,18,21,22,23))) { // varchar
            $itemRascunho[$key] = !empty($value) ? "'".$value."'" : 'null';
        } elseif($key == 20) {
            $itemRascunho[$key] = "'".$value."'";
        } else if($key == 19) {
            $itemRascunho[$key] = $_SESSION['_cusupocodi_'];
        } elseif(empty($value)) {
            $itemRascunho[$key] = 'null';
        } else {
            $itemRascunho[$key] = $value;
        }
    }
