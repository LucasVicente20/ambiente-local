 ...
 * Data:     15/02/2007
 * Objetivo: Acrescentado funcionalidade para novos especificações do custo (Fardamento,Limpeza e Didático)
 *           Ajuste para registrar movimentacoes com DataBase e nao com a Data de Fechamento
 * -------------------------------------------------------------------------------------------------------------
 * Alterado: Carlos Abreu
 * Data:     25/05/2007
 * Objetivo: Alteração nas Movimentações 33 e 34 para lançamento em custo e contabil com valores das diferencas
 ...
 * Data:     20/12/2007
 * Objetivo: Adição de filtro no select para só fechar o inventário que estiver aberto, pois antes atualizava todos
 * -------------------------------------------------------------------------------------------------------------
 * Alterado: Rodrigo Melo
 * Data:     26/12/2007 
 * Objetivo: Correção de erro de SQL no filtro do select feito fechar o inventário que estiver aberto.
 * -------------------------------------------------------------------------------------------------------------
 * Alterado: Rodrigo Melo
 * Data:     02/05/2007
 * Objetivo: Ajuste nas movimentações para chamar a rotina de lançamento contábil.
 * -------------------------------------------------------------------------------------------------------------
 * Alterado: Ariston Cordeiro
 * Data:     09/07/2008
 * Objetivo: Valor informado em consolidação agora refere-se à quantidade real do estoque (AARMATQTDE), ao invés da quantidade total (AARMATESTR)
 * -------------------------------------------------------------------------------------------------------------
 * Alterado: Ariston Cordeiro
 * Data:     23/10/2008
 * Objetivo: Permitir que itens de estoques zerados não precisem de contagem/recontagem
 * -------------------------------------------------------------------------------------------------------------
 * Alterado: Rodrigo Melo
 * Data:     11/08/2009
 * Objetivo: Permitir que almoxarifados que não realizaram movimentações no ano corrente possam realizar o fechamento do inventário periódico.
 * -------------------------------------------------------------------------------------------------------------
 * Alterado: Lucas Baracho
 * Data:     16/01/2023
 * Objetivo: Tarefa Redmine 277665
 * -------------------------------------------------------------------------------------------------------------
 * Alterado: Lucas Baracho
 * Data:     20/01/2023
 * Objetivo: Tarefa Redmine 277826
 * -------------------------------------------------------------------------------------------------------------
 */
        ...
        $res  = $db->query($sql);
                ...
                for( $i=0;$i<$Rows; $i++ ){
                  ...
                  if ($Linha[5]=="N"){
                      ...
                      if( PEAR::isError($resarmazenamento) ){
                          $db->query('ROLLBACK');
                          $db->query('END TRANSACTION');
                          ExibeErroBD("$ErroPrograma\nLinha: ".__LINE__."\nSql: $sql");
                          $db->disconnect();
                  ...
                  if ($Linha[1]>0 and $Linha[2]==0){
                      ...
                      if( PEAR::isError($resmovimentacao) ){
                          $db->query('ROLLBACK');
                          $db->query('END TRANSACTION');
                          ExibeErroBD("$ErroPrograma\nLinha: ".__LINE__."\nSql: $sql");
                          $db->disconnect();
                  ...
                  if ($QtdDiferenca>0){
                    ...
                    $db->query('END TRANSACTION');
							...
							if( PEAR::isError($resalmoxarifado) ){
								...
								$Mens     = 1;
								$Tipo     = 1;
								$Mensagem = "Nenhum material foi movimentado neste ano - Inventário Periódico Concluído com Sucesso";
								...
								<td>
									<table class="textonormal" border="0" align="left" width="100%" summary="">
										...
										<tr>
											...
											$DataBase = date("d/m/Y");
											?>
											<td class="textonormal"><input type="text" name="DataBase" value="<?php echo $DataBase; ?>" maxlength="10" size="10" class="textonormal" disabled>
										...
										<!--<tr>
											...
											?>
											<td class="textonormal"><input type="text" name="DataBase" value="<?php $DataBase?>" maxlength="10" size="10" class="textonormal">
