		...
		for ($i = 0; $i < count($ItemCarga) and !$Rollback; $i++) {
			...
			if (!$Movimentado) {
				...
				if ($QtdEst == "") {
					...
					$Quantidade = str_replace(",",".",$QtdEst);
				}

				if ($ValorUnit == "") {
					$Valor = "NULL";
				} else {
					$Valor = str_replace(",",".",$ValorUnit);
				}
				...
				if (PEAR::isError($resest)) {
					...
					if ($Qtd == 0) {
						...
						$sql .= "            CLOCMACODI, CMATEPSEQU, AINVCOANOB, AINVCOSEQU, AINVMAESTO, VINVMAUNIT, TINVMAULAT ";
						$sql .= ") VALUES ( ";
						$sql .= "            $Localizacao, $Material, $Ano, $Sequencial, $Quantidade, $Valor, '" . date("Y-m-d H:i:s") . "' ";
						$sql .= " ) ";
...
<script language="javascript" type="">
	...
	function AbreJanelaItem(url,largura,altura) {
		if (!document.CadInventarioInicialContagem.Almoxarifado.value) {
			document.CadInventarioInicialContagem.submit();
		} elseif (!document.CadInventarioInicialContagem.Localizacao.value) {
