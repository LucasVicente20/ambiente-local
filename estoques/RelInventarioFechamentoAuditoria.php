...
#-----------------------------------------------------------------------------------------
# Portal da DGCO
# Programa: RelInventarioFechamentoAuditoria.php
# Autor:    Carlos Abreu
# Data:     13/12/2006
...
AddMenuAcesso( '/estoques/RelInventarioFechamentoAuditoria.php' );
AddMenuAcesso( '/estoques/RelInventarioFechamentoAuditoriaPdf.php' );
...
if($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		$Ano_Sequencial_DataBase = $_POST['Ano_Sequencial_DataBase'];
		list($Ano,$Sequencial,$DataBase) = explode("_",$Ano_Sequencial_DataBase);
		$Ordem                   = $_POST['Ordem'];
}else{
...
if($Botao == "Limpar"){
		header("location: RelInventarioFechamentoAuditoria.php");
		exit;
		...
		if( ($Almoxarifado == "") && ($CarregaAlmoxarifado == 'N') ){
				...
				$Mens      = 1;
				$Tipo      = 2;
				$Mensagem .= "<a href=\"javascript:document.RelInventarioFechamentoAuditoria.Almoxarifado.focus();\" class=\"titulo2\">Almoxarifado</a>";
		}
		if( ($Localizacao == "") && ($CarregaLocalizacao == 'N') ){
				...
				$Mens      = 1;
				$Tipo      = 2;
				$Mensagem .= "<a href=\"javascript:document.RelInventarioFechamentoAuditoria.Localizacao.focus();\" class=\"titulo2\">Localização</a>";
		} elseif($Ano_Sequencial_DataBase=="") {
				if($Mens == 1){ $Mensagem .= ", "; }
				$Mens      = 1;
				$Tipo      = 2;
				$Mensagem .= "<a href=\"javascript:document.RelInventarioFechamentoAuditoria.Ano_Sequencial.focus();\" class=\"titulo2\">Data Base</a>";
		}
		if($Ordem == 0){
				if($Mens == 1){ $Mensagem .= ", "; }
				$Mens      = 1;
				$Tipo      = 2;
				$Mensagem .= "<a href=\"javascript:document.RelInventarioFechamentoAuditoria.Ordem.focus();\" class=\"titulo2\">Ordem</a>";
		}
		if($Mens == 0){
				$Url = "RelInventarioFechamentoAuditoriaPdf.php?Almoxarifado=$Almoxarifado&Localizacao=$Localizacao&Ordem=$Ordem&Ano=$Ano&Sequencial=$Sequencial&DataBase=$DataBase&".mktime();
...
function enviar(valor){
	document.RelInventarioFechamentoAuditoria.Botao.value=valor;
	document.RelInventarioFechamentoAuditoria.submit();
}
...
<form action="RelInventarioFechamentoAuditoria.php" method="post" name="RelInventarioFechamentoAuditoria">
<br><br><br><br><br>
<table cellpadding="3" border="0" summary="">
	...
	<tr>
		...
		<td align="left" class="textonormal" colspan="2">
			...
			<a href="../index.php"><font color="#000000">Página Principal</font></a> > Estoques > Inventário > Relatórios > Acertos / Justificativas
		</td>
	</tr>
	<!-- Fim do Caminho-->
						...
						<table border="1" cellpadding="3" cellspacing="0"  bordercolor="#75ADE6" summary="" class="textonormal" width="100%">
							<tr>
								<td align="center" bgcolor="#75ADE6" valign="middle" colspan="4" class="titulo3">
									RELATÓRIO DE ACERTOS / JUSTIFICATIVAS
								</td>
							</tr>
							<tr>
								<td class="textonormal">
									<p align="justify">
										Para imprimir os dados de Acertos / Justificativas do Almoxarifado, preencha os campos abaixo e clique no botão "Imprimir".
								...
								<td class="textonormal">
									<table class="textonormal" border="0" align="left" summary="" width="100%">
										...
										if($Localizacao != ""){
												...
												echo "<select name=\"Ano_Sequencial_DataBase\" class=\"textonormal\">\n";
												$sql  = "SELECT AINVCOANOB, AINVCOSEQU, TO_CHAR(TINVCOBASE,'DD/MM/YYYY') ";
												...
												if( PEAR::isError($res) ){
														...
														while ( $Linha = $res->fetchRow()){
																echo "<option value=\"$Linha[0]_$Linha[1]_$Linha[2]\">$Linha[2]</option>\n";
														}
												}
												echo "</select>\n";
												echo "</td>\n";
