...
# Objetivo: Programa de consolidação do inventário periódico
# Autor:    Carlos Abreu
# Data:     22/11/2006
#----------------------------------------
...
# Data:     04/07/2008	- Verificação para proibir se algum item possui a quantidade de consolidação nula
# Alterado: Ariston Cordeiro
# Data:     31/10/2008		- Antes, caso fosse gerado a consolidação uma 2a vez, a nova consolidação era gerada em cima
#														da consolidação anterior, através de updates. Isto gerava inconsistência em casos que um item contado na
#														Consolidação antiga não eram contados na consolidação nova (pois a nova mantinha a contagem da
#														consolidação anteriror). Agora a consolidação está simplesmente sendo deletada.
# Alterado: Rodrigo Melo
# Data:     15/12/2008  -  Alteração para exibir a quantidade do estoque (AINVMAESTO em SFPC.INVENTARIOMATERIAL).
# Alterado: Rodrigo Melo
# Data:     11/07/2009  -  Correção para não exibir erro ao tentar consolidar sem a inclusão de nenhum materiais na contagem/recontagem.
# Alterado: Ariston Cordeiro
# Data:     03/11/2009  -  CR3335- Adicionando mensagem "Nenhum item contado ou recontado."
# Alterado: Ariston Cordeiro
# Data:     03/11/2009  -  CR3335- Adicionando mensagem de erro "Não há nenhum item para consolidação. Caso o almoxarifado não possua itens e não houve nenhuma movimentação desde o inventário anterior, ele deve ser finalizado sem contagem."
#-------------------------------------------
...
if($Botao == "Limpar"){
		...
		if(db::isError($res)){
			...
			/*$sql="
				...
				WHERE
					im.clocmacodi = $Localizacao
					and im.ainvcoanob = $Ano
					and im.ainvcosequ = $Sequencial
					and im.cmatepsequ not in (
						select ir.cmatepsequ from SFPC.TBinventarioregistro ir
						where
							ir.clocmacodi = im.clocmacodi
							and ir.ainvcoanob = im.ainvcoanob
							and ir.ainvcosequ = im.ainvcosequ
					)
			";
			$res  = $db->query($sql);
			if(db::isError($res)){
						...
						if(db::isError($res)){
								...
								if( $Rows != 0 ){
										...
										for($Row=1;$Row<=$Rows;$Row++){
												...
												/*
												# Antes estava registrando a consolidação atual por cima da consolidação anterior, por updates.
												# Isto gera inconsistências em situações em que materiais contados na consilidação anterior não são contados na nova consolidação
												$sql  = "SELECT * ";
												...
												$resInv  = $db->query($sql);
												if(db::isError($resInv)){
														ExibeErroBD("$ErroPrograma\nLinha: ".__LINE__."\nSql: $sql");
														$db->disconnect();
														exit;
												}else{
														$RowsInv = $resInv->numRows();
														if ( $RowsInv > 0 ){
																if ($Linha[2]==$Linha[3]){
																		# Repete quantidade da Recontagem
																		...
																		if ( $Linha[2]=="" ){
																				...
																				$sql .= "   SET AINVMAESTO = ".str_replace(",",".",$Linha[2]).", ";
																		}
																		...
																		# Coloca a quantidade da informada na consolidação
																		$sql  = "UPDATE SFPC.TBINVENTARIOMATERIAL ";
																		if ( $Consolidado[array_search($Linha[0],$Codigo)]==""){
																				$sql .= "   SET AINVMAESTO = NULL, ";
																		} else {
																				$sql .= "   SET AINVMAESTO = '".str_replace(",",".",$Consolidado[array_search($Linha[0],$Codigo)])."', ";
																		}
																		$sql .= "       CGREMPCODI = ".$_SESSION['_cgrempcodi_'].", ";
																		$sql .= "       CUSUPOCODI = ".$_SESSION['_cusupocodi_'].", ";
																		$sql .= "       TINVMAULAT = '$datahora'";
			...
			<table  border="0" cellspacing="0" cellpadding="3" width="100%" summary="">
				<tr>
	  	      	...
	  	      	<tr>
         	      	...
         	      	<input type="hidden" name="Posicao" id="Posicao" value="<?=$Posicao;?>">
            		</td>
		        	</tr>
    	  	  </table>
					</td>
