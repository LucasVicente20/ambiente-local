...
# --------------------------------------------------------------------------------
# Portal da DGCO
# Programa: CadRequisicaoMaterialAlterarIncluir.php
# Autor:    Roberta Costa
# Data:     12/09/2005
# Objetivo: Programa de Alteração de Requisição de Material - Incluir Item(ns)
...
# Data:     12/08/2008 - Tratamento de erro quando um item for especificado com valor 0
# --------------------------------------------------------------------------------
# Alterado: Rodrigo Melo
# Data:     24/04/2009 - Resolvendo CR794 - Alterando a alteração de requisição de material para que seja possível incluir e excluir itens na mesma funcionalidade.
# --------------------------------------------------------------------------------
# Alterado: Ariston Cordeiro
# Data:     14/03/2010 - Adicionando checagem se a requisição é do tipo 'Requisitante', e se a situação está como 'em análise', para evitar erro em que o usuário recarrega a página e altera uma requisição já atendida.
# --------------------------------------------------------------------------------
			...
			if ($Mens == 0) {
				...
				if ($Mens == 0) {
					...
					for ($j=0;$j<count($MaterialOrdem);$j++) {
						$Dados       = split($SimboloConcatenacaoArray,$MaterialOrdem[$j]);
						$SeqMaterial = $Dados[1];
						$QtdSol      = $Dados[2];
						$NovaOrdem++;
...
if (count($_SESSION['item']) != 0) {
	...
	if ($ItemRequisicao == "") {
		...
		for ($i=0; $i<count($ItemRequisicao); $i++) {
			$DadosItem            = split($SimboloConcatenacaoArray,$ItemRequisicao[$i]);
														...
														<table border="1" cellpadding="3" cellspacing="0" bgcolor="#bfdaf2" bordercolor="#75ADE6" width="100%" summary="">
															...
															<tr>
																...
																<td class="textonormal" width="80%">
																	...
																	<?	$Url = "CadItemDetalhe.php?ProgramaOrigem=CadRequisicaoMaterialAlterarIncluir&Material=$Material[$i]";
																		if (!in_array($Url,$_SESSION['GetUrl'])) {
																			$_SESSION['GetUrl'][] = $Url;
																		}
																	?>
																	<a href="javascript:AbreJanela('<?=$Url;?>',730,370);">
																		<font color="#000000">
																			<?php	$Descricao = split($SimboloConcatenacaoDesc,$DescMaterial[$i]);
																					echo $Descricao[1];
																			?>
																		</font>
																	</a>
															...
															<tr>
																<td class="textonormal" colspan="5" align="center">
																	<?	$Url = "CadIncluirItem.php?ProgramaOrigem=CadRequisicaoMaterialAlterarIncluir&PesqApenas=E&Zerados=N&Almoxarifado=$Almoxarifado";
																		if (!in_array($Url,$_SESSION['GetUrl'])) {
																			$_SESSION['GetUrl'][] = $Url;
																		}
																	?>
																	<input type="button" name="IncluirItem" value="Incluir Item" class="botao" onclick="javascript:AbreJanelaItem('<?=$Url;?>',730,350);">
