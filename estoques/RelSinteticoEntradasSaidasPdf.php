...
# Alterado: Álvaro Faria
# Data:     21/12/2007 - Relatório que exibe as informações a partir de 2008, baseado no vmovmaumed (Correção no select para pegar o valor médio nas movimentações entre almoxarifados)
...
# Data:     24/02/2010 - Recolocando a ordenação do select do valor médio por código da movimentação, de modo que seja pego sempre a última movimentação daquele dia
#----------------------------------
...
function EstoqueData($Almoxarifado, $TipoMaterial, $DataPesquisa, $db){
		...
		$sql .= "    AND MOV.DMOVMAMOVI > '$DataPesquisa 23:59:59' ";
		...
		if( PEAR::isError($res) ){
				...
				if($rows == 0){
						...
						for($i=0; $i< $rows; $i++){
								...
								$sqlvalor .= "    AND DMOVMAMOVI = ";
								$sqlvalor .= "        (SELECT MAX(DMOVMAMOVI) FROM SFPC.TBMOVIMENTACAOMATERIAL ";
								$sqlvalor .= "          WHERE CALMPOCODI = $Almoxarifado ";
								$sqlvalor .= "            AND CMATEPSEQU = ".$CodigoReduzido[$i]." ";
								$sqlvalor .= "            AND DMOVMAMOVI <= '$DataPesquisa 23:59:59' ";
								$sqlvalor .= "            AND (FMOVMASITU IS NULL OR FMOVMASITU = 'A') )";
...
function Movimentacoes($Almoxarifado, $TipoMov, $TipoMaterial, $DataPesquisa1, $DataPesquisa2, $Movimentacoes, $Operacao, $db){
		...
		$sql .= "    AND GRU.FGRUMSTIPM = '$TipoMaterial' ";
		$sql .= "    AND MOV.CALMPOCODI = $Almoxarifado ";
		$sql .= "    AND MOV.DMOVMAMOVI > '$DataPesquisa1 23:59:59' ";
		if ($DataPesquisa2){
				$sql .= "    AND MOV.DMOVMAMOVI <= '$DataPesquisa2 23:59:59' ";
		}
