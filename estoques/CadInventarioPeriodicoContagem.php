 ...
 * Data:     04/01/2008
 * Objetivo: Alteração para tornar mais rápido o inclusão e remoção de itens.
 * -----------------------------------------------------------------------------------------------------------------------------
 * Alterado: Rodrigo Melo
 * Data:     26/02/2008
 * Objetivo: Alteração para mudar a exibição da mensagem "MATERIAL NÃO CADASTRADO NO ALMOXARIFADO" para
 *           "MATERIAL NÃO CADASTRADO OU INATIVO NO ALMOXARIFADO" e não permitir que materiais inativos sejam contados
 * -----------------------------------------------------------------------------------------------------------------------------
 * Alterado: Ariston Cordeiro
 * Data:     16/06/2008
 * Objetivo: Ao incluir item, agora o campo 'Código reduzido' recebe foco.
 * -----------------------------------------------------------------------------------------------------------------------------
 * Alterado: Lucas Baracho
 * Data:     18/01/2023
 * Objetivo: Tarefa Redmine 277837
 * -----------------------------------------------------------------------------------------------------------------------------
 */
...
function MaterialDescricao($Db, $Localizacao, $CodigoMaterial) {
	$ErroPrograma = "CadInventarioPeriodicoContagem";
...
if ($Botao == "Limpar") {
	...
	if ($Mens == 0) {
		...
		$sql .= "   	AND A.AINVCOANOB = (SELECT	MAX(AINVCOANOB) ";
		...
		<table cellpadding="3" border="0" summary="">
			...
			<tr>
				...
				<td align="left" class="textonormal" colspan="2">
					...
					</a> > Estoques > Inventário > Periódico > Contagem/Recontagem
				</td>
			</tr>
			<!-- Fim do Caminho-->
			<!-- Erro -->
										...
										<td>
											<table class="textonormal" border="0" align="left" width="100%" summary="">
												...
												if ($Almoxarifado != "") {
													...
													if ($Localizacao) {
														if (!$Etapa or !$Cpf) {
															...
															<tr>
																...
																<td class="textonormal">
																	<input type="text" class="textonormal" name="Cpf" id="Cpf" value="<?php echo $Cpf; ?>" size="11" maxlength="11" onblur="enviar('CarregaDados');">
													...
													?>
													<input type="text" class="textonormal" name="Responsavel" id="Responsavel" value="<?php echo $Responsavel; ?>" size="40" maxlength="60">
												</td>
											</tr>
											<tr>
					        			<td colspan="2">
																								...
																								${'valor'.$Row} = str_replace(".",",",$Linha[1]);
					     																		...
					     																		<table id="<?php echo "tabela".$Row;?>" border="0" class="textonormal" cellpadding="3" cellspacing="1" width="100%">
                                               														<tr bgcolor="DDECF9">
                                                 														...
                                                 														<td width="25%">
                                                   															<input type="text" class="textonormal" id="<?php echo "dado".$Row; ?>" name="<?php echo "dado".$Row; ?>" value="<?php echo ${'dado'.$Row} ?>" size="14" onBlur="javascript:makeRequest('ajax/AjaxDescricaoMaterial.php', '?Codigo='+document.getElementById('CadInventarioPeriodicoContagem').dado<?php echo $Row; ?>.value, 'descricao<?php echo $Row; ?>');">
                                                 														</td>
                                                 														<td width="25%">
																			...
																			if (($Botao=="CarregarDados1") or ($Botao=="Excluir")) {
					     														for ($pos=1;$pos<=$Posicao;$pos++) { // l
					       															if ($Botao=="CarregarDados1") {
				             															...
				             															<?php 
					       															} elseif (($Botao=="Excluir") and (['check'.$pos]!="on")) {
					       																$Row++; 
					       																?>
					      																<table id="<?php echo "tabela".$Row; ?>" border="0" class="textonormal" cellpadding="3" cellspacing="1" width="100%">
                                               												<tr bgcolor="DDECF9">
                                                 												...
                                                 												<td width="25%">
                                                   													<input type="text" class="textonormal" id="<?php echo "dado".$Row; ?>" name="<?php echo "dado".$Row; ?>" value="<?php echo ['dado'.$pos] ?>" size="14" onBlur="javascript:makeRequest('ajax/AjaxDescricaoMaterial.php', '?Codigo='+document.getElementById('CadInventarioPeriodicoContagem').dado<?php echo $pos; ?>.value, 'descricao<?php echo $pos; ?>');">
                                                 												</td>
                                                 												<td width="25%">
