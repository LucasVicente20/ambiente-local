...
#--------------------------------------------------------------------------------------------------
# Portal da DGCO
# Programa: CadNotaFiscalMaterialManter.php
# Objetivo: Programa que Seleciona a Nota Fiscal para Inclusão/Exclusão de Itens da Nota Fiscal
...
# Data:      10/09/2008 - Removendo acesso a SFPC.TBFORNECEDORESTOQUE
# ------------------------------------------------------------------------
...
AddMenuAcesso( '/estoques/CadNotaFiscalMaterialManterIncluir.php' );
AddMenuAcesso( '/estoques/CadNotaFiscalMaterialManterExcluir.php' );
...
if($Botao == "Limpar"){
		...
		if( ($Almoxarifado == "") && ($CarregaAlmoxarifado == 'N') ){
				...
				$Tipo      = 2;
				$FlgSistema = 1;
				$Mensagem .= "<a href=\"javascript:document.CadNotaFiscalMaterialManter.Almoxarifado.focus();\" class=\"titulo2\">Almoxarifado</a>";
		}
		$MensErro = ValidaPeriodo($DataIni,$DataFim,$Mens,"CadNotaFiscalMaterialManter");
		if( $MensErro != "" ){ $Mensagem .= $MensErro; $Mens = 1; $Tipo = 2; $FlgSistema = 1; }
		if( ($NumNota) and (!SoNumeros($NumNota)) ){
				if( $Mens == 1 ){ $Mensagem .= ", "; }
				$Mens      = 1;
				$Tipo      = 2;
				$Mensagem .= "<a href=\"javascript:document.CadNotaFiscalMaterialManter.NumNota.focus();\" class=\"titulo2\">Número da Nota Válido</a>";
		}
}
if($Botao == "Validar"){
		$Mens     = 0;
		...
		if( $MaterialDescricaoDireta != "" and $OpcaoPesquisaMaterial == 0 and ! SoNumeros($MaterialDescricaoDireta) ){
				...
				$Mens = 1;
				$Tipo = 2;
				$Mensagem .= "<a href=\"javascript:document.CadNotaFiscalMaterialManter.txtmaterialdireta.focus();\" class=\"titulo2\">Código reduzido do Material</a>";
				...
				$Mens = 1;
				$Tipo = 2;
				$Mensagem .= "<a href=\"javascript:document.CadNotaFiscalMaterialManter.txtmaterialdireta.focus();\" class=\"titulo2\">Descrição com no mínimo 2 caracteres</a>";
		}
}
...
function enviar(valor){
	document.CadNotaFiscalMaterialManter.Botao.value = valor;
	document.CadNotaFiscalMaterialManter.submit();
}
function validapesquisa(){
		if(document.CadNotaFiscalMaterialManter.txtmaterialdireta.value != "") {
				document.CadNotaFiscalMaterialManter.Botao.value = "Validar";
				document.CadNotaFiscalMaterialManter.submit();
		}
}
...
<form action="CadNotaFiscalMaterialManter.php" method="post" name="CadNotaFiscalMaterialManter">
<br><br><br><br><br>
			...
			<table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#75ADE6" summary="" class="textonormal"  bgcolor="#FFFFFF">
				...
				<tr>
					<td class="textonormal" colspan="4">
						<p align="justify">
							Para Alterar uma Nota Fiscal cadastrada, informe os campos abaixo, clique no botão "Pesquisar" e clique no número da Nota Fiscal desejada.
						</p>
					</td>
				</tr>
				<tr>
						...
						<table border="0" width="100%" summary="">
							...
							<tr>
								...
								<td class="textonormal">
									...
									$URLIni = "../calendario.php?Formulario=CadNotaFiscalManterManter&Campo=DataIni";
									$URLFim = "../calendario.php?Formulario=CadNotaFiscalManterManter&Campo=DataFim";
									?>
							...
							<tr>
								...
								<td class="textonormal">
									<input type="radio" onclick="javascript:submit();" name="Operacao" value="I" <?php if( $Operacao == "I" ){ echo "checked"; } ?> checked> Incluir Item(ns)
									<input type="radio" onclick="javascript:submit();" name="Operacao" value="E" <?php if( $Operacao == "E" ){ echo "checked"; } ?> > Excluir Item(ns)
								</td>
							</tr>
						</table>
					</td>
						...
						if( PEAR::isError($res) ){
								...
								if( $Qtd > 0 ){
										...
										while( $Linha	= $res->fetchRow() ){
												...
												if($Operacao == 'I'){
														$Url = "CadNotaFiscalMaterialManterIncluir.php?Almoxarifado=$Almoxarifado&AnoNota=$AnoNota&NotaFiscal=$NotaFiscal";
														echo "		<a href=\"$Url\"><font color=\"#000000\">".$NumeroNota."/".$AnoNota."</font></a>";
												}else{
														$Url = "CadNotaFiscalMaterialManterExcluir.php?Almoxarifado=$Almoxarifado&AnoNota=$AnoNota&NotaFiscal=$NotaFiscal";
														echo "		<a href=\"$Url\"><font color=\"#000000\">".$NumeroNota."/".$AnoNota."</font></a>";
												}
