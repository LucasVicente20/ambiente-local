...
# Objetivo: Exibir as diferenças dos Valores Contábeis gerados entre o Postgresql e o Oracle.
#           Para cada movimentação de estoque (TBMOVIMENTACAOMATERIAL), verifica na tabela de 
#           lançamento contábil (ORACLE - SFCT.TBMOVCONTABILALMOXARIFADO) a existência dos lançamentos 
#						contábeis de acordo com a tabela de parâmetros contabéis (postgresql - SFPC.TBMOVCONTABILALMOXARIFADOPARAM)
#           No caso de saída por requisição a data de movimentação é baseada na data da situação (tipo=5 - baixada) 
#           da requisição em questão.
# Autor:    Carlos Abreu
# Data:     16/02/2007
# Alterado: Carlos Abreu
# Data:     18/04/2007 - Ajuste para corrigir data para movimentacoes (21 e 22)
# Alterado: Carlos Abreu
...
if($Botao == "Limpar"){
		header("location: ConsMovimentacaoCompararContabil.php");
		exit;
...
<form action="ConsMovimentacaoCompararContabil.php" method="post" name="ConsMovimentacaoCompararContabil">
<br><br><br><br><br>
					...
					<td class="textonormal">
						<table border="1" cellpadding="3" cellspacing="0" bordercolor="#75ADE6" class="textonormal" summary="">
							...
							if($Botao == "Comparar" and $Mens == 0){
									...
									$sqlpost  = "SELECT ";
									$sqlpost .= "       CASE WHEN A.CREQMASEQU IS NOT NULL AND A.CTIPMVCODI NOT IN (2,21,22) THEN TO_DATE(TO_CHAR(J.TSITRESITU,'YYYY-MM-DD'),'YYYY-MM-DD') ELSE A.DMOVMAMOVI END AS DATAMOV, ";
									...
									$sqlpost .= "       )";
									$sqlpost .= "       * ";
									$sqlpost .= "       CASE WHEN A.CTIPMVCODI = 4 THEN ";
									$sqlpost .= "       ( ";
									$sqlpost .= "       SELECT VMOVMAVALO FROM SFPC.TBMOVIMENTACAOMATERIAL ";
									$sqlpost .= "        WHERE CMATEPSEQU = A.CMATEPSEQU ";
									$sqlpost .= "          AND CREQMASEQU = A.CREQMASEQU ";
									$sqlpost .= "          AND (FMOVMASITU IS NULL OR FMOVMASITU = 'A' ) ";
									$sqlpost .= "          AND TMOVMAULAT = ( ";
									$sqlpost .= "                SELECT MAX(TMOVMAULAT) FROM SFPC.TBMOVIMENTACAOMATERIAL ";
									$sqlpost .= "                 WHERE CTIPMVCODI IN (4,19,20) ";
									$sqlpost .= "                   AND CMATEPSEQU = A.CMATEPSEQU ";
									$sqlpost .= "                   AND CREQMASEQU = A.CREQMASEQU ";
									$sqlpost .= "                   AND (FMOVMASITU IS NULL OR FMOVMASITU = 'A' ) ";
									$sqlpost .= "              ) ";
									$sqlpost .= "       ) ";
									...
									$sqlpost .= " WHERE A.CTIPMVCODI NOT IN (3,5,7,8,18,19,20,31) ";
									...
									for ($Row=0;$Row<count($Caixa);$Row++){
											...
											if ($LinhaAnterior <> array($Linha[0],$Linha[4],$Linha[6],$Linha[21],$Linha[22],$Linha[14]) ){
													$LinhaAnterior = array($Linha[0],$Linha[4],$Linha[6],$Linha[21],$Linha[22],$Linha[14]);
													list($dmovmamovi_ano,$dmovmamovi_mes,$dmovmamovi_dia) = explode("-",$Linha[0]);
