...
#-------------------------------------------------------------------------
# Portal da DGCO
# Programa: CadMaterialPrecoPesquisaSelecionar.php
# Autor:    Heraldo Botelho ( Copiado de CadMaterialIncluirSelecionar.php ) Rossana Lira
# Objetivo: Programa de Seleção de Manutenção de Pesquisa de Mercado   
#-------------------------------------------------------------------------
...
# Adiciona páginas no MenuAcesso #
AddMenuAcesso( '/materiais/CadMaterialPrecoPesquisaIncluir.php' );
AddMenuAcesso( '/materiais/CadMaterialPrecoPesquisaExcluir.php' );
...
if( $_SERVER['REQUEST_METHOD'] == "POST" ){
		...
		$Mens                     = $_GET['Mens'];
		$Tipo                     = $_GET['Tipo'];
		$Acao                     = $_GET['Acao']; //1 - Incluir Pesquisa  2 - Manter Pesquisa
		$ExcluidoComSucesso =	$_GET['ExcluidoComSucesso'];		
}

if ($ExcluidoComSucesso ==1 ) {
	$Mens=1;
	$Tipo=1;
...
if($Botao == "Limpar"){
	    if ($acao==1)
	         $paramHeader="location:CadMaterialPrecoPesquisaSetarSelecionar1.php";
	    elseif ($acao==1)
	         $paramHeader="location:CadMaterialPrecoPesquisaSetarSelecionar2.php"; 
		...
		if( $MaterialDescricaoDireta != "" and $OpcaoPesquisaMaterial == 0 and ! SoNumeros($MaterialDescricaoDireta) ){
				...
				$Mens = 1;
				$Tipo = 2;
				$Mensagem .= "<a href=\"javascript:document.CadMaterialPrecoPesquisaSelecionar.MaterialDescricaoDireta.focus();\" class=\"titulo2\">O código reduzido do Material</a>";
				...
				$Mens = 1;
				$Tipo = 2;
				$Mensagem .= "<a href=\"javascript:document.CadMaterialPrecoPesquisaSelecionar.MaterialDescricaoDireta.focus();\" class=\"titulo2\">Descrição com no mínimo 2 caracteres</a>";
		}
		if( $SubclasseDescricaoDireta != "" and $OpcaoPesquisaSubClasse == 0 and ! SoNumeros($SubclasseDescricaoDireta) ){
				if( $Mens == 1 ){ $Mensagem .= ", "; }
				$Mens = 1;
				$Tipo = 2;
				$Mensagem .= "<a href=\"javascript:document.CadMaterialPrecoPesquisaSelecionar.SubclasseDescricaoDireta.focus();\" class=\"titulo2\">O código reduzido da Subclasse</a>";
				...
				$Mens = 1;
				$Tipo = 2;
				$Mensagem .= "<a href=\"javascript:document.CadMaterialPrecoPesquisaSelecionar.SubclasseDescricaoDireta.focus();\" class=\"titulo2\">Descrição com no mínimo 2 caracteres</a>";
		}
}
?>
<html>
...
function enviar(valor){
	document.CadMaterialPrecoPesquisaSelecionar.Botao.value = valor;
	document.CadMaterialPrecoPesquisaSelecionar.submit();
}
function checktodos(){
	document.CadMaterialPrecoPesquisaSelecionar.Material.value = '';
	document.CadMaterialPrecoPesquisaSelecionar.submit();
}
function remeter(){
	document.CadMaterialPrecoPesquisaSelecionar.submit();
...
function validapesquisa(){
	if( (document.CadMaterialPrecoPesquisaSelecionar.MaterialDescricaoDireta.value != '') ||
	    (document.CadMaterialPrecoPesquisaSelecionar.SubclasseDescricaoDireta.value != '') ) {
		if( document.CadMaterialPrecoPesquisaSelecionar.Grupo ){
	   	document.CadMaterialPrecoPesquisaSelecionar.Grupo.value = '';
	 	}
		if (document.CadMaterialPrecoPesquisaSelecionar.Classe ){
	 	  document.CadMaterialPrecoPesquisaSelecionar.Classe.value = '';
	 	}
		if( document.CadMaterialPrecoPesquisaSelecionar.Botao ){
  		 	document.CadMaterialPrecoPesquisaSelecionar.Botao.value = 'Validar';
  	}
	}
	if( document.CadMaterialPrecoPesquisaSelecionar.MaterialDescricaoFamilia ){
	  	if( document.CadMaterialPrecoPesquisaSelecionar.MaterialDescricaoFamilia.value != '' ){
   				document.CadMaterialPrecoPesquisaSelecionar.Material.value = '';
  		}
	}
  document.CadMaterialPrecoPesquisaSelecionar.submit();
...
<script language="JavaScript">Init();</script>
 <?php   if ($acao==1) {   ?>
		<form action="CadMaterialPrecoPesquisaSetarSelecionar1.php" method="post" name="CadMaterialPrecoPesquisaSelecionar">
 <?php    }  ?>		
 <?php   if ($acao==2) {  ?>
		<form action="CadMaterialPrecoPesquisaSetarSelecionar2.php" method="post" name="CadMaterialPrecoPesquisaSelecionar">
 <?php   }  ?>
...
<table cellpadding="3" border="0" width="100%" summary="">
  ...
  <tr>
    ...
    <td align="left" class="textonormal" colspan="2">
      ...
      <?php if ($acao==1) { ?>
	      <a href="../index.php"><font color="#000000">Página Principal</font></a> > Materiais > Preço > Pesquisa Mercado > Incluir
      <?php } elseif ($acao==2) { ?>
	      <a href="../index.php"><font color="#000000">Página Principal</font></a> > Materiais > Preço > Pesquisa Mercado > Excluir
      <?php }  ?>
	...
	<tr>
        ...
        <tr>
          <td align="center" bgcolor="#75ADE6" valign="middle" class="titulo3" colspan="2">
	           PESQUISA DE MERCADO - PREÇOS DE MATERIAIS
          </td>
        </tr>
        <tr>
									  ...
									  if( $qtdres > 0 ){
									      ...
									      echo "	         <td class=\"titulo3\" bgcolor=\"#F7F7F7\" width=\"90%\">DESCRIÇÃO DO MATERIAL</td>\n";
												...
												while( $row	= $res->fetchRow() ){
														...
														if ( $acao==1) {
															$Url = "CadMaterialPrecoPesquisaIncluir.php?Material=$MaterialSequ";
														}
														elseif ( $acao==2) {
															$Url = "CadMaterialPrecoPesquisaExcluir.php?Material=$MaterialSequ";
														}
