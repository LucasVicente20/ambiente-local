...
if( $Botao == "Excluir" ){
	...
	if( $Critica == 1 ) {
		...
		if( !eregi("\.zip$", $_FILES['NomeArquivo']['name']) && !eregi("\.pdf$", $_FILES['NomeArquivo']['name']) &&
			!eregi("\.rtf$", $_FILES['NomeArquivo']['name']) && !eregi("\.doc$", $_FILES['NomeArquivo']['name']) &&
			!eregi("\.xls$", $_FILES['NomeArquivo']['name']) && !eregi("\.txt$", $_FILES['NomeArquivo']['name']) &&
			!eregi("\.sdw$", $_FILES['NomeArquivo']['name']) && !eregi("\.jpg$", $_FILES['NomeArquivo']['name']) &&
			!eregi("\.bmp$", $_FILES['NomeArquivo']['name'])
		){
			$Mens = 1; $Tipo = 2;
			$Mensagem .= "Arquivos com a extensão .zip, .jpg, .bmp, .pdf, .rtf, .doc, .xls, .txt ou .sdw";
		}
		...
		<tr>
			...
			<table border="0" summary="">
	            ...
	            <tr>
					...
					<textarea name="ArquivoDescricao" cols="39" rows="5" OnKeyUp="javascript:ncaracteres(1)" OnBlur="javascript:ncaracteres(0)" OnSelect="javascript:ncaracteres(1)" class="textonormal"><?php echo $ArquivoDecricao;?></textarea>
	            </tr>
				<tr>
