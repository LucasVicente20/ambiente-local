...
/**
 * Portal da DGCO
 * Programa: RotEnvioCartilhaGuiaManual.php
...
$NomePrograma = "RotEnvioCartilhaGuiaManual.php";
...
if ($_POST['Botao'] == "Enviar") {
    ...
    if ($Mens == 0) {
        ...
        $sql = "
			SELECT MAX(CDOCPOCODI) FROM SFPC.TBDOCUMENTACAOPORTAL
		";
        $res = $db->query($sql);
        if (PEAR::isError($res)) {
            $db->query("ROLLBACK");
            ...
            $sql = "
				INSERT INTO
					...
					(
						...
						" . $_SESSION['_cusupocodi_'] . ", '" . date("Y-m-d H:i:s") . "', '" . $ArquivoDestinoNome . "', 'C'
					)
			";
            $res = $db->query($sql);
            if (PEAR::isError($res)) {
...
function enviar(valor){
	document.formRotEnvioCartilhaGuiaManual.Botao.value=valor;
	document.formRotEnvioCartilhaGuiaManual.submit();
}
...
function CaracteresDescricao(valor){
	formRotEnvioCartilhaGuiaManual.NCaracteresDescricao.value = '' +  formRotEnvioCartilhaGuiaManual.Descricao.value.length;
	if (formRotEnvioCartilhaGuiaManual.Descricao.value.length >= 200) {
		formRotEnvioCartilhaGuiaManual.NCaracteresDescricao.value = 200;
		var texto = formRotEnvioCartilhaGuiaManual.Descricao.value;
		formRotEnvioCartilhaGuiaManual.Descricao.value = formRotEnvioCartilhaGuiaManual.Descricao.value.substr(0,200);
	}
}
<?php MenuAcesso(); ?>
//-->
...
<body background="../midia/bg.gif" marginwidth="0" marginheight="0">
	...
	<form enctype="multipart/form-data" id="formRotEnvioCartilhaGuiaManual"
		name="formRotEnvioCartilhaGuiaManual"
		action="RotEnvioCartilhaGuiaManual.php" method="post">
		<br> <br> <br> <br>
		<table cellpadding="3" border="0" summary="">
			...
			<tr>
				...
				<td align="left" class="textonormal" colspan="2"><br> <font
					...
					Cartilhas, Guias e Manuais > Envio de Documentação</td>
			</tr>
			<!-- Fim do Caminho-->
								...
								<table border="1" cellpadding="3" cellspacing="0"
									...
									<tr>
										<td align="center" bgcolor="#75ADE6" valign="middle"
											class="titulo3">ENVIO DE CARTILHAS, GUIAS E MANUAIS</td>
									</tr>
									<tr>
										<td class="textonormal">
											<p align="justify">Selecione o documento desejado. O
												documento deve ser menor que 5Mb e de um dos tipos: .zip,
												.pdf, .rtf, .doc, .xls, odp, odt, sdw, ppt ou .txt</p>
										</td>
									</tr>
									<tr>
										<td>
											<table class="textonormal" border="0" align="left" summary="">
												<tr>
													...
													<td class="textonormal"><input type="file"
														name="NomeArquivo" class="textonormal" size="50"
														value="<?=$NomeArquivo?>" /></td>
												</tr>
												<tr>
													<td class="textonormal" bgcolor="#DCEDF7" height="20">Título<span style="color: red;">*</span></td>
													<td class="textonormal"><input type="text" name="Titulo"
														class="textonormal" size="50" maxlength="99"
														value="<?=$Titulo?>" /></td>
												</tr>
												<tr>
													<td class="textonormal" bgcolor="#DCEDF7" height="20">Descrição<span style="color: red;">*</span></td>
													<td class="textonormal"><font class="textonormal">máximo de
															200 caracteres</font> <input type="text"
														name="NCaracteresDescricao" disabled readonly size="3" value="0"
														class="textonormal"><br> <textarea name="Descricao"
															id="Descricao" class="textonormal" cols="39" rows="3"
															OnKeyUp="javascript:CaracteresDescricao(1)"
															OnBlur="javascript:CaracteresDescricao(0)"
															OnSelect="javascript:CaracteresDescricao(1)"><?=$Descricao?></textarea>
									...
									<tr>
										<td class="textonormal" align="right"><input type="button"
											value="Enviar" class="botao"
											onclick="javascript:enviar('Enviar');"> <input type="button"
											value="Voltar" class="botao"
											onclick="javascript:enviar('Voltar');"> <input type="hidden"
											name="Botao" value=""></td>
									</tr>
								</table>
							</td>
