...
// Data:     08/10/2018
// Objetivo: Tarefa Redmine 204986
// ---------------------------------------------------------------------------
...
// Remover justificativas salvas anteriormente na sessão
if(!empty($_SESSION['arrJustf'])) {
    unset($_SESSION['arrJustf']);
}
...
if ($Botao == "Voltar") {
    ...
    if ($erro) {
        ...
        $Tipo = 0;
        $_SESSION['arrJustf'] = $arrJustf; // Salvar justificativas que foram editadas para recuperar
        $Botao = '';
    } else {
        $db->query("COMMIT");
        ...
        if(!empty($_SESSION['arrJustf'])) {
            unset($_SESSION['arrJustf']); // Remover justificativas
        }
        header("Location: $Url");
        exit;
    }
                                    ...
                                    <table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#75ADE6" summary="" class="textonormal" bgcolor="#FFFFFF">
                                        ...
                                        $sql  = "SELECT DISTINCT
                                                 ...
                                                 WHERE
                                                    ...
                                                    AND TRP.CORGLICODI  = $Orgao
                                                    AND TRP.FTRPREVALI IS NOT NULL
                                                    AND (MAT.FMATEPNTRP = 'N' OR MAT.FMATEPNTRP IS NULL)
                                                 ORDER BY
                                        ...
                                        if (PEAR::isError($result)) {
                                            ...
                                            while ($Linha = $result->fetchRow()) {
                                                ...
                                                <td bgcolor="#DCEDF7">
                                                    <input type="text" 
                                                    value="<?php echo (!empty($_SESSION['arrJustf'][$Linha[5]][$Linha[6]])) ? $_SESSION['arrJustf'][$Linha[5]][$Linha[6]] : $Linha[10]; ?>" 
                                                    id="arrJustf[<?php echo $Linha[5]?>][<?php echo $Linha[6]?>]" 
                                                    name="arrJustf[<?php echo $Linha[5]?>][<?php echo $Linha[6];?>]" 
