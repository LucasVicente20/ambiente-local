...
class Report  {
    ...
    public function DadosReportPuro() {
        ...
        $sql .= " informacoesAta.aitarporde, informacoesAta.cmatepsequ, informacoesAta.citarpitel, CASE WHEN solco.fsolcocont = 'S' THEN 'SIM' ELSE 'NÃO' END as geracontrato, contr.ectrpcnumf as numerocontrato,  informacoesAta.NumAtaa FROM ";
        $sql .= " sfpc.tbitemlicitacaoportal iteml INNER JOIN (SELECT lpad(cast(centro.ccenpocorg as varchar),2,'0')||lpad(cast(centro.ccenpounid as varchar),2,'0') ";
        ...
        $sql .= " atarpint.ccomlicodi, atarpint.cgrempcodi FROM sfpc.tbataregistroprecointerna atarpint, sfpc.tbitemataregistropreconova itemata, (select distinct ccenpocorg, ccenpounid, corglicodi ";
        $sql .= " from sfpc.tbcentrocustoportal where acenpoanoe = 2022 AND fcenpositu = 'A') as centro WHERE atarpint.carpnosequ = itemata.carpnosequ AND  atarpint.corglicodi = centro.corglicodi  AND atarpint.farpincorp = 'S' ";
        ...
        $sql .= " GROUP BY fasel.CLICPOPROC, fasel.ALICPOANOP, fasel.CGREMPCODI, fasel.CCOMLICODI, fasel.CORGLICODI ) as maiorordem WHERE iteml.alicpoanop >= 2018 AND iteml.alicpoanop < 2022 AND iteml.vitelpvlog is not null AND iteml.cmatepsequ = matep.cmatepsequ ";
        ...
        $sql .= " and iteml.corglicodi = licit.corglicodi and iteml.ccomlicodi = licit.ccomlicodi and iteml.cgrempcodi = licit.cgrempcodi and licit.cmodlicodi = modal.cmodlicodi ORDER BY 1,2 ";
    ...
    // public function DadosReportParticipante($carpnosequ , $CMATEPSEQU , $citelpsequ) {
    //     $sql ="( SELECT itemata.carpnosequ AS NumAta, orgao.eorglidesc AS orgaoAta, itemata.citarpsequ AS ItemAta, resultadoscc.corglicodi AS OrgaoParticipante, SUM ( resultadoscc.aitescqtso ) AS somaparticipante ";
    ...
    //     $sql .=" AND itemata.carpnosequ = ".$carpnosequ." AND itemata.CMATEPSEQU = ".$CMATEPSEQU." AND itemata.citarpsequ = ".$citelpsequ." AND resultadoscc.corglicodi = orgao.corglicodi GROUP BY ";
    ...
    //     $sql .=" AND itemata.carpnosequ = ".$carpnosequ." AND itemata.CMATEPSEQU = ".$CMATEPSEQU." AND itemata.citarpsequ = ".$citelpsequ." AND itemparticipanterp.corglicodi = orgao.corglicodi GROUP BY NumAta, orgaoAta, ItemAta, OrgaoParticipante )";
    //     $resultado = executarSQL($this->con, $sql);
    ...
    // public function DadosReportCarona($carpnosequ  , $CMATEPSEQU) {
    //     $sql = "(SELECT itemata.carpnosequ as NumAtaC, orgao.eorglidesc as orgaoAta, itemata.citarpsequ as ItemAtaC, resultadoscc.corglicodi AS OrgaoCarona, SUM(resultadoscc.aitescqtso) AS somacarona FROM sfpc.tbitemataregistropreconova itemata ";
    ...
    //     $sql .= " resultadoscc.aitescqtso IS NOT NULL AND itemata.carpnosequ = ".$carpnosequ." AND itemata.CMATEPSEQU = ".$CMATEPSEQU." AND resultadoscc.corglicodi = orgao.corglicodi GROUP BY NumAtaC, orgaoAta, ItemAtaC, OrgaoCarona ) UNION ( SELECT itemcaronarp.carpnosequ as NumAtaC, orgao.eorglidesc as orgaoAta,";
    //     $sql .= " itemcaronarp.citarpsequ as ItemAtaC, itemcaronarp.corglicodi AS OrgaoCarona, SUM(itemcaronarp.aitcrpqtut) AS somacarona FROM sfpc.tbitemataregistropreconova itemata LEFT JOIN sfpc.tbitemcaronainternaatarp ";
    ...
    //     $sql .= " AND itemcaronarp.corglicodi IS NOT NULL AND itemata.CMATEPSEQU = ".$CMATEPSEQU." AND itemcaronarp.corglicodi = orgao.corglicodi GROUP BY NumAtaC, orgaoAta, ItemAtaC, OrgaoCarona ) ";
    ...
    public function DadosReportCarona($carpnosequ  , $CMATEPSEQU, $citelpsequ) {
        ...
        $sql .= " itemcaronarp ON itemata.carpnosequ = itemcaronarp.carpnosequ AND itemata.citarpsequ = itemcaronarp.citarpsequ AND ( itemcaronarp.aitcrpqtut <> 0 AND itemcaronarp.aitcrpqtut <> 0.0000 ), sfpc.tborgaolicitante as orgao WHERE itemata.carpnosequ = ".$carpnosequ." ";
        $sql .= " AND itemcaronarp.corglicodi IS NOT NULL AND itemata.CMATEPSEQU = ".$CMATEPSEQU." AND itemata.citarpsequ = ".$citelpsequ." AND itemcaronarp.corglicodi = orgao.corglicodi GROUP BY NumAtaC, orgaoAta, ItemAta, OrgaoCarona ) ";
