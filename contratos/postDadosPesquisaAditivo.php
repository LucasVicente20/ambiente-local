...
// require_once "ClassMedicaoPesquisar.php";
require_once "ClassAditivoPesquisar.php";
// A classe AditivoPesquisar() foi criada para abstrair a função de pesquisa por ser diferente em cada tela. CR#238403
$ObjContrato = new ContratoPesquisar();
// $ObjMedicao = new MedicaoPesquisar();
$ObjAditivo = new AditivoPesquisar();
$arrayTirar  = array('.',',','-','/');
...
switch($_POST['op']){
    ...
    case "PesquisaContrato":
        ...
        }else{$cnpj = str_replace($arrayTirar,'',$_POST['cnpj']);
        $Cpf  = str_replace($arrayTirar,'',$_POST['cpf']);
        $tudook = true;
        ...
        $dadosTabela = $ObjAditivo->Pesquisar($ArrayDados);
        ...
        foreach($dadosOrgao as $infOrgao){
                foreach($dadosTabela as $informacoesTable){
                    if($informacoesTable->eorglidesc == $infOrgao->eorglidesc){
                        ...
                        $situacaoForne = $ObjContrato->GetSituacaoFornecedor($informacoesTable->aforcrsequ);
                        $TipoCompra    = $ObjContrato->GetTipoCompra($informacoesTable->ctpcomcodi);
                        $SituacaoContrato  = $ObjAditivo->GetSituacaoContrato($informacoesTable->cfasedsequ);
