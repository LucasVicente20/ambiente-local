...
# -------------------------------------------------------------------------
# Portal da Compras
# Programa: CadAditivoManterPesquisar.php
...
<script language="javascript" type="">
				...
				if(orgao[k].eorglidesc != '' && orgao[k].eorglidesc != 'undefined'){
					...
					for(i in objJson){
						if(orgao[k].eorglidesc == objJson[i].eorglidesc){
							...
							tableHtml     +='<td width="14%"> <a href="javascript:selecionar('+objJson[i].cdocpcsequ+')">'+numeroContrato+'</a></td>';
							tableHtml     +='<td width="50%">'+objJson[i].ectrpcobje+'</td>';
							tableHtml     +='<td width="22%">'+objJson[i].nforcrrazs+'</td>';
	...
	function selecionar(registro){
		...
		if(registro != '' && registro != null && registro != 'undefined' ){
			$("#registrocontrato").val(registro);
			$("#formPesquisa").attr('action','./CadAditivoManter.php');
			$("#formPesquisa").submit();
		}
	}
	<!--
    $(document).ready(function() {
		...
		$.post("postDadosPesquisaAditivo.php",{op:"OrgaoGestor"}, function(data){
        ...
        $('#radio-cpf').on('click',function(){
            ...
            $("#mostracpf").show();
			$("#mostracnpj").hide();
			//$("#mostracnpj").css("marginTop", "-20px");
        });
        $('#radio-cnpj').on('click',function(){
			$("#mostracnpj").show();
			$("#cpf").val('');
            $("#mostracpf").hide();
			//$("#mostracpf").css("marginTop", "-20px");
        });
        $("#btnLimpa").on('click', function(){
            window.location.reload();
        });
        $("#btnIncluir").on('click', function(){
            window.location.href="./CadAditivoIncluir.php";
        });
        $("#btnPesquisar").on('click', function(){
			...
			$.post("postDadosPesquisaAditivo.php", //Substituição do postDadosPesquisaMedição CR#238403
					{
						op					:"PesquisaContrato",
						cpf					:$("#cpf").val(),
						cnpj				:$("#cnpj").val(),
        ...
        $(".btn-pesquisa-fornecedor").on('click', function(){
			const documento = $("input[name='cpf-cnpj']:checked").val();
			$.post("postDadosPesquisaAditivo.php",{op:"Fornecerdor",cpf:$("#cpf").val(),cnpj:$("#cnpj").val(),doc:documento}, function(data){
...
<body background="../midia/bg.gif" marginwidth="0" marginheight="0">
	...
	<input type="hidden" name="idregistro" id="registrocontrato">
