...
require_once "ClassRelatorioContratoPesquisar.php";

$ObjContrato = new ContratoPesquisar();
$ObjRelCont = new RelatorioContratoPesquisar();
$arrayTirar  = array('.',',','-','/');
...
switch($_POST['op']){
    ...
    case "Pesquisa":
        ...
        $dadosTabela = $ObjRelCont->Pesquisar($ArrayDados);
        ...
        foreach($dadosOrgao as $infOrgao){
                foreach($dadosTabela as $informacoesTable){
                    if($informacoesTable->eorglidesc == $infOrgao->eorglidesc){
                        ...
                        $SituacaoContrato  = $ObjRelCont->GetSituacaoContrato($informacoesTable->cfasedsequ);
        ...
        if(empty($dadosTabela)){
            $objJS = json_encode(array("status" => false, "msm" => "Não há contratos que satisfaçam os critérios de pesquisa informados"));
            print($objJS);
            $tudook =false;
            exit;
        }
