...
#-------------------------------------------------------------------------
# Portal da DGCO
# Programa: CadApostilamentoAlterar.php
# Autor:    Eliakim Ramos | Edson Dionisio
# Data:     23/04/2020
# Objetivo: Programa de Incluir/Alterar Apostilamento
#-------------------------------------------------------------------------
...
include "../funcoes.php";
require_once "ClassApostilamentoAlterar.php";
...
$objApostilamentoAlterar = new ApostilamentoAlterar();
$ObjApostilamento = new ClassApostilamento();
$ApostilamentoGeral = new ApostilamentoGeral();
$ObjContratoManter = new ContratoManter();

$situacaoApostilamento = $objApostilamentoAlterar->get_situacaoApostilamento();

if(isset($_GET['idregistro'])) { //existe source?
    $param = $_GET['idregistro'];
}
else {
    $param = "";
}
...
if($_GET['idregistro'] == NULL){
    $idregistro = $_SESSION['ssid'];

}else{
     $_SESSION['ssid'] = $_GET['idregistro'];
     $idregistro = $_SESSION['ssid'];
}
if (isset($idregistro)) {
    $_SESSION['ssid'] = $idregistro;
} else {
    $idregistro = $_SESSION['ssid'];
}

$apostilamento = $objApostilamentoAlterar->getApostilamento($idregistro);
$situacao = $apostilamento[0]->cfasedsequ;
...
$cpf_gestor = $apostilamento[0]->napostcpfg;
$cpf_gestor = $apostilamento[0]->napostcpfg;
$codContrato = $objApostilamentoAlterar->getContrato($cdocpcseq2);
$fornApost = $objApostilamentoAlterar->getFornecedorApostilamento($cdocpcseq2, $num_apostilamento);
//Eliakim Ramos
$dadosTiposApostilamentos = $ObjApostilamento->getTiposApostilamentos();
$dadosHtmlSelectTipoApostilamento = $ObjApostilamento->MontaSelectBoxTipoApostilamento($dadosTiposApostilamentos,$num_tipo_apostilamento);
session_start();
...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    if(!empty($_SESSION['fiscal_selecionado_incluir'])){
        $DDFF = $ApostilamentoGeral->getDocumentosFicaisEFicalApost($idregistro);
        $i=0;
        foreach($DDFF as $k){
          foreach($_SESSION['fiscal_selecionado_incluir'] as $f){
                if($k->fiscalcpf == $f->fiscalcpf){
                    ...
                    $fiscalselecionado[] = (object)  array(
                        ...
                        'fiscalcpf'       => $ApostilamentoGeral->MascarasCPFCNPJ($f->fiscalcpf),
               ...
               $fiscalselecionado[] = (object)  array(
                                ...
                                'fiscalcpf'       => $ApostilamentoGeral->MascarasCPFCNPJ($k->fiscalcpf),
...
if(!empty($_SESSION['documento_anexo_incluir'])){
    ...
    unset($_SESSION['documento_anexo_incluir']);
    $DadosDocAnexo = $objApostilamentoAlterar->GetDocumentosAnexos($idregistro);
...
$aforcrsequ = $fornApost->seqfornecedor;

    if($aforcrsequ != NULL){
    $RazaoSocial = $fornApost->razao;
    $logradouro = $fornApost->rua;
    $compl = $fornApost->complemento;
    $bairro = $fornApost->bairro;
    $cidade = $fornApost->cidade;
    $estado = $fornApost->estado;
    $numero = $fornApost->numero; 
    $_SESSION['dadosContratado']  = $ApostilamentoGeral->getFornecedorDados($aforcrsequ);
 }
    
    if(!empty($fornApost->cnpj)){
        $CnpjCpf_forn = $objApostilamentoAlterar->mask($fornApost->cnpj, '##.###.###/####-##');
        $CnpjCpf = $CnpjCpf_forn;
    }

    if(!empty($fornApost->cpf)){
        $CnpjCpf_forn = $objApostilamentoAlterar->mask($fornApost->cpf, '###.###.###-##');
        $CnpjCpf = $CnpjCpf_forn;
    }
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($csolcosequ) && !empty($aforcrsequ)) {
        $_SESSION['dadosContratado']  = $ApostilamentoGeral->getFornecedorDados($aforcrsequ);
        $_SESSION['dadosObjOrgao']    = $ApostilamentoGeral->GetOrgaoEDescObj($csolcosequ);
    ...
    if (!is_null($_POST['sccselec-' . $_POST['seqScc']])) {
        $_SESSION['origemScc']       = $objApostilamentoAlterar->corrigeString($_POST['origselec-' . $_POST['seqScc']]);
  ...
  //  $_SESSION['fiscal_selecionado_incluir'] = $DadosDocFiscaisFiscal;
...
if ($CNPJ_CPF == "") {
    ...
    if ($CnpjCpf == "") {
        ...
        $Mens      = 1;
        $Tipo      = 2;
        $Mensagem .= "<a href=\"javascript:document.CadApostilamentoAlterar.CnpjCpf.focus();\" class=\"titulo2\">$TipoDocumento</a>";
    } else {
        if ($CNPJ_CPF == 1) {
            ...
            if ($valida_cnpj === false) {
                ...
                $Mens      = 1;
                $Tipo      = 2;
                $Mensagem .= "<a href=\"javascript:document.CadApostilamentoAlterar.CnpjCpf.focus();\" class=\"titulo2\">CNPJ Válido</a>";
            }
        } else {
            ...
            if ($valida_cpf === false) {
                ...
                $Mens      = 1;
                $Tipo      = 2;
                $Mensagem .= "<a href=\"javascript:document.CadApostilamentoAlterar.CnpjCpf.focus();\" class=\"titulo2\">CPF Válido</a>";
            }
        }
    }
...
$_SESSION['codigo_forn'] = empty($_SESSION['codigo_forn']) ? $fornApost->seqfornecedor :  $_SESSION['codigo_forn'];
?>
<html>
<?php

//$dadosTipoCompra = $objApostilamentoAlterar->ListTipoCompra();
...
<script language="javascript" type="">
        ...
        function enviar(valor){
            console.log(document.CadApostilamentoAlterar.Botao);
            document.CadApostilamentoAlterar.submit();
        }
        ...
        function Submete(Destino){
            document.CadApostilamentoAlterar.Destino.value = Destino;
            document.CadApostilamentoAlterar.submit();
        }
        ...
        function formatarCampo(campoTexto) {
            ...
            if (campoTexto.length <= 11) {
                var cpf = mascaraCpf(campoTexto);
                return campoTexto = cpf;
            } else {
                return campoTexto = mascaraCnpj(campoTexto);
            }
        }
      ...
      //  }
        function mascaraCpf(valor) {
            //console.log(valor.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g,"\$1.\$2.\$3\-\$4"));
        ...
        function verifica(value){
            ...
            valorRetroativoApostilamento.disabled = true;
            valorApostilamento.disabled = true;
        ...
        $(document).ready(function() {
            ...
            seleciona_opcao('<?php echo $num_tipo_apostilamento; ?>');
            //console.log(teste);
            ...
            $("#btnvoltar").on('click', function(){
                contrato = <?php echo $cdocpcseq2; ?>;
                $("#formContratoAlterar").attr("action","CadApostilamentoManter.php");
                $("#formContratoAlterar").submit();
            });
           ...
           $("#btnSalvarModal").live("click",function(){
            ...
            $("#btnRemoveAnexo").live("click",function(){
               ...
               // console.log(docanexselec);
                $.post("postDadosApostilamentoGeral.php",{op:"RemoveDocAnex",marcador:docanexselec},function(data){
            ...
            $("#salvarApostilamento").on('click', function(){
                ...
                $.post("postDadosApostilamentoAlterar.php", $("#formContratoAlterar").serialize(), function(data){ 
...
<body background="../midia/bg.gif" marginwidth="0" marginheight="0">
    ...
    <form action="CadApostilamentoAlterar.php" method="post" id="formContratoAlterar" name="CadApostilamentoAlterar">
        <input type="hidden" name="op" value="alterarApostilamento">
        <input type="hidden" name="idregistro" value="<?php echo $idregistro; ?>">
        <input type="hidden" name="idcontrato" value="<?php echo $cdocpcseq2; ?>">
        ...
        <table cellpadding="3" border="0" summary="">
                    ...
                    <table border="0" cellspacing="0" cellpadding="3" summary="" width="1024px" bgcolor="#FFFFFF">
                                            ...
                                            <table cellpadding="0" cellspacing="0" border="0" bordercolor="#75ADE6" width="1024px" summary="">
                                                <tr bgcolor="#bfdaf2">
                                                    ...
                                                    <table class="textonormal" id="scc_material" summary="" bordercolor="#75ADE6" style="border: 1px solid #75ade6; border-radius: 4px;" width="100%">
                                                        <thead>
                                                            <td class="titulo3" colspan="17" align="center" bgcolor="#75ADE6" valign="middle"> <b>MANTER APOSTILAMENTO</b></td>
                                                        </thead>
                                                        <tbody>
                                                            <tr>
                                                                <td bgcolor="#DCEDF7" class="textonormal" width="225px">Número do Contrato/Ano
                                                                <td bgcolor="White">
                                                                    <?php echo $codContrato[0]->ectrpcnumf; ?>
                                                                </td>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="textonormal" class="textonormal">
                                            <input style="font-size: 10.6667px;" type="text" id="num_apostilamento" name="num_apostilamento" value="<?php echo $num_apostilamento; ?>">
                                        </td>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td bgcolor="#DCEDF7" class="textonormal" width="225px">Situação do Apostilamento
                                ...
                                <select id="selectContrato" name="situacao_apostilamento" size="1" title="situacao_apostilamento" style="width:301px; font-size: 10.6667px; ">	
                                        <!-- <option class="textonormal" style="font-size: 10.6667px;" value="" ></option> -->
                                        <?php foreach($situacaoApostilamento as $key => $tipo){ 
                                            $selecionadoTipo = ($tipo->cfasedsequ == $situacao) ? 'selected="selected"' : '';    
                                    ?>
                                        <option class="textonormal" style="font-size: 10.6667px;" value="<?php echo $tipo->cfasedsequ;?>" <?php echo $selecionadoTipo; ?> >
                                            <?php echo $tipo->esitdcdesc; ?>
                                        </option>
                                    <?php } ?>
                                </select>
                            </td>
                        ...
                        <tr>
                            ...
                            <td class="textonormal" class="textonormal">
                                <input style="font-size: 10.6667px;" class = "dinheiro4casas" type="text" id="valor_retroativo_apostilamento"  name="valor_retroativo_apostilamento" value="<?php echo $valor_retro_apostilamento; ?>" disabled>
                            </td>
                        </tr>
                        <tr>
                            <td bgcolor="#DCEDF7" class="textonormal" width="50px">Valor do Apostilamento</td>
                            <td class="textonormal" class="textonormal">
                                <input style="font-size: 10.6667px;" class = "dinheiro4casas" type="text" id="valor_apostilamento" name="valor_apostilamento" value="<?php echo $valor_apostilamento; ?>"  disabled>
            ...
            <tr>
                ...
                <td class="textonormal">
                    <input id="dataPublicacaoDom" type="text" name="dataPublicacaoDom" style="font-size: 10.6667px;" class="data" value="<?php echo  date('d/m/Y', strtotime($data_apostilamento)); ?>" maxlength="10" size="12" title="">
                    <a style="text-decoration: none" href="javascript:janela('../calendario.php?Formulario=CadApostilamentoAlterar&amp;Campo=dataPublicacaoDom','Calendario',220,170,1,0)">
            ...
            <tr>
                ...
                <td bgcolor="White" class="textonormal">
                    <input type="text" id="gestorNome" name="gestorNome" maxlength="120" style="width:315px; font-size: 10.6667px;" value="<?php echo $nome_gestor; ?>" disabled>
                </td>
                </td>
            </tr>
            <tr>
                ...
                <td bgcolor="White" class="textonormal">
                    <input type="text" name="gestorMatricula" id="gestorMatricula" maxlength="20" size="10" style="font-size: 10.6667px;" value="<?php echo $mat_gestor; ?>" disabled>
                </td>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td bgcolor="White" class="textonormal">
                    <input class="CPF" type="text" name="gestorCPF" id="gestorCPF" size="10" style="font-size: 10.6667px;" value="<?php echo $cpf_gestor; ?>" disabled>
                </td>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td bgcolor="White" class="textonormal">
                    <input type="text" id="gestorEmail" name="gestorEmail" maxlength="60" style="font-size: 10.6667px; width:173px; text-transform: none;" value="<?php echo $email_gestor; ?>" disabled>
                </td>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td bgcolor="White" class="textonormal">
                    <input class="telefone" type="text" name="gestorTelefone" id="gestorTelefone" style="font-size: 10.6667px;" size="13" value="<?php echo $tel_gestor; ?>" disabled>
                </td>
                </td>
            </tr>
            <tr>
