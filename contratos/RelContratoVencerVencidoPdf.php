...
AddMenuAcesso('/contratos/RelContratoVencerVencidoPdf.php');
...
if($cpfCnpj == 'CNPJ'){
	$fornecedor = str_replace($arrayTirar,'',$cnpj);
	$sql_forn = " and forn.aforcrccgc = '" . $fornecedor ."'";
	$sql_desc_forn = " aforcrccgc = '" . $fornecedor ."'";
}else{
	$fornecedor = str_replace($arrayTirar,'',$cpf);
...
if(empty($_SESSION['vencido-vencer'])){
	$vencidoVencer = $_POST['vencido-vencer'];
	$_SESSION['vencido-vencer'] = $vencidoVencer;
}else{
	$vencidoVencer = $_SESSION['vencido-vencer'];
}
...
$sql  .= (!empty($periodo)) ? $sql_periodo_novo : '';
					...
					$existe_registro_orgao = $existe->fetchRow();
					 ...
					 if($existe_registro_orgao[0] > 0){ //&& ($reg_row[$key][30] == $orgao_licitante)
							...
							foreach ($registros as $key => $value) {
								...
								// 	$saldo_a_executar = 'R$ '. number_format((floatval($existe_valor_registro[1])) + floatval($soma_valor_adt[0]) + floatval($soma_valor_apost[0])  - floatval($existe_valor_med[0]), 4, ',', '.');
								// }
								$valor_global_aditivo_apostilamento = $objFuncoesGerais->valorGlobal($teste[$key][0]);
								$saldo_a_executar = $objFuncoesGerais->saldoAExecutar($teste[$key][0]);
