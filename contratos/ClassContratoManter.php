...
# -------------------------------------------------------------------------
# Portal da Compras
# Programa: ClassContratoManter.php
...
# -------------------------------------------------------------------------
    ...
    Class ContratoManter {
        ...
        public function GetOrgao(){
            ...
            $usuarioEspecial = $this->checaUsuarioManterEspecial($cusupocodi);
        ...
        //BUSCA FISCAIS DO CONTRATO CONTRATO CONSOLIDADO ALTERADOS POR APOSTILAMENTO IMPLANTADO 
        public function getFicaisContratoConsolidado($CodSequContrato){
            $sql = " SELECT * FROM sfpc.tbapostilamento as apost
            ...
            WHERE apost.cdocpcseq2 = " .$CodSequContrato. " and apost.ctpaposequ in (2,3) and doc.ctidocsequ = 3 
            and doc.cfasedsequ = 6 and doc.csitdcsequ = 1 and (select max(a2.aapostnuap) from sfpc.tbapostilamento as a2 
            inner join sfpc.tbdocumentosfpc as doc2 on (doc2.cdocpcsequ = a2.cdocpcsequ) where a2.cdocpcseq2 = apost.cdocpcseq2 and a2.ctpaposequ in (2,3)
            and doc2.ctidocsequ = 3 and doc2.cfasedsequ = 6 and doc2.csitdcsequ = 1 ) = apost.aapostnuap and fisc.nfiscdmlfs is not null ";
            $resultado = executarSQL($this->conexaoDb, $sql);
            $dadosRetorno = array();
            while($resultado->fetchInto($retorno, DB_FETCHMODE_OBJECT)){
                $dadosRetorno[] = $retorno;
        ...
        public function GetDocumentosAnexos($codDoc,$sequdocanexo=null,$edcanxnome=null){
            $sql  = " SELECT  cdocpcsequ ,cdocpcsequ AS sequdocumento, cdcanxsequ AS sequdocanexo, edcanxnome AS nomearquivo, tdcanxcada AS datacadasarquivo, tdcanxulat AS ultimasatualizacao, ";
        ...
        //madson
        public function PesquisaItensSCC($csolcosequ, $aforcrsequ, $tipoCompra, $citelpnuml){
        ...
        //ÚLTIMO GESTOR IMPLANTADO DO APOSTILAMENTO DO CONTRATO CONSOLIDADO 
        public function GetGestorContratoConsolidado($CodSequContrato){
            $sql = " select apost.* from sfpc.tbapostilamento as apost inner join sfpc.tbdocumentosfpc as doc on (doc.cdocpcsequ = apost.cdocpcsequ)
                     where apost.cdocpcseq2 = ".$CodSequContrato." and apost.ctpaposequ in (2,3) and doc.ctidocsequ = 3 and doc.cfasedsequ = 6 and doc.csitdcsequ = 1 and 
                     (select max(a2.aapostnuap) from sfpc.tbapostilamento as a2 inner join sfpc.tbdocumentosfpc as doc2 on (doc2.cdocpcsequ = a2.cdocpcsequ) 
                     where a2.cdocpcseq2 = apost.cdocpcseq2 and a2.ctpaposequ in (2,3) and doc2.ctidocsequ = 3 and doc2.cfasedsequ = 6 and doc2.csitdcsequ = 1 
                     and a2.napostmlgt is not null and a2.napostmlgt != '' ) = apost.aapostnuap " ;
                    $resultado = executarSQL($this->conexaoDb, $sql);
                    $resultado->fetchInto($retorno, DB_FETCHMODE_OBJECT);
        ...
        public function UpdateContrato($dadosContratos){
            ...
            $cctrpcccep = !empty($dadosContratos['cctrpcccep'])?"  cctrpcccep='".$dadosContratos['cctrpcccep']."', ":" ";
            $nctrpccdrl=!empty($dadosContratos['nctrpccdrl'])?" nctrpccdrl='".$dadosContratos['nctrpccdrl']."', ":" ";
            $nctrpcedrl=!empty($dadosContratos['nctrpcedrl'])?" nctrpcedrl='".$dadosContratos['nctrpcedrl']."', ":" ";
            $nctrpcnarl=!empty($dadosContratos['nctrpcnarl'])?" nctrpcnarl='".$dadosContratos['nctrpcnarl']."', ":" ";
            $nctrpcprrl=!empty($dadosContratos['nctrpcprrl'])?" nctrpcprrl='".$dadosContratos['nctrpcprrl']."', ":" ";
            $nctrpcmlrl=!empty($dadosContratos['nctrpcmlrl'])?" nctrpcmlrl='".$dadosContratos['nctrpcmlrl']."', ":" ";
            $ectrpclogr=!empty($dadosContratos['ectrpclogr'])?" ectrpclogr='".$dadosContratos['ectrpclogr']."', ":" ";
            $aforcrsequ=!empty($dadosContratos['aforcrsequ'])?" aforcrsequ='".$dadosContratos['aforcrsequ']."', ":" ";
            $aforcrseq1=!empty($dadosContratos['aforcrsequ'])?" aforcrseq1='".$dadosContratos['aforcrsequ']."', ":" "; // campo de fornecedor original
            $actrpcnuen=!empty($dadosContratos['actrpcnuen'])?" actrpcnuen='".$dadosContratos['actrpcnuen']."', ":" ";
            $ectrpccomp= !empty($dadosContratos['ectrpccomp'])?" ectrpccomp='".$dadosContratos['ectrpccomp']."', ":" ";
            $ectrpcbair= !empty($dadosContratos['ectrpcbair'])?" ectrpcbair='".$dadosContratos['ectrpcbair']."', ":" ";
            $nctrpccida=!empty($dadosContratos['nctrpccida'])?" nctrpccida='".$dadosContratos['nctrpccida']."', ":" ";
            $cctrpcesta=!empty($dadosContratos['cctrpcesta'])?" cctrpcesta='".$dadosContratos['cctrpcesta']."', ":" ";
            $ectrpctlct=!empty($dadosContratos['ectrpctlct'])?" ectrpctlct='".$dadosContratos['ectrpctlct']."', ":" ";
            $ectrpcnumf=!empty($dadosContratos['ectrpcnumf'])?$dadosContratos['ectrpcnumf']:"Aguardando Numeração";
            $dctrpcinvg = $dadosContratos['dctrpcinvg'];
            $csolcosequ = !empty($dadosContratos['csolcosequ'])?" csolcosequ='".$dadosContratos['csolcosequ']."', ":' ';
            $ctpcomcodi =!empty($dadosContratos['ctpcomcodi'])?" ctpcomcodi='".$dadosContratos['ctpcomcodi']."', ":" ";
            $actrpcnuap = !empty($dadosContratos['actrpcnuap'])?" actrpcnuap='".$dadosContratos['actrpcnuap']."', ":" ";
            $actrpcnuad = !empty($dadosContratos['actrpcnuad'])?" actrpcnuad='".$dadosContratos['actrpcnuad']."', ":" ";
            //$valGlobalAdApost = !empty($dadosContratos['valGlobalAdApost'])?" vctrpceant='".$dadosContratos['valGlobalAdApost']."', ":" ";  // Estava assim por  achar que estava errado comentei e coloquei a linha a baixo
            $valGlobalAdApost = !empty($dadosContratos['valGlobalAdApost'])?" vctrpcglaa='".$dadosContratos['valGlobalAdApost']."', ":" ";
            $valExecutado = isset($dadosContratos['valExecutado'])?" vctrpceant='".$dadosContratos['valExecutado']."', ":" ";  // estava comentado e eu descomentei e troquei o empty por isset  para ele poder aceitar o 0
            $vctrpcvlor = !empty($dadosContratos['vctrpcvlor'])?" vctrpcvlor='".$dadosContratos['vctrpcvlor']."', ":" ";
            //retirei da query quando for null pois o banco não estava reconhecendo o null na query
            $actrpcanoc = !empty($dadosContratos['actrpcanoc'])?" actrpcanoc= ".$dadosContratos['actrpcanoc'].", " :" ";


            $sql  =" UPDATE sfpc.tbcontratosfpc SET actrpcnumc = ".$dadosContratos['actrpcnumc'].", ectrpcnumf= '".$ectrpcnumf."', ectrpcobje='".$dadosContratos['ectrpcobje']."', ";
            ...
            $sql .= $actrpcanoc." cctrpcopex='".$dadosContratos['cctrpcopex']."', actrpcpzec=".$dadosContratos['actrpcpzec'].$dctrpcpbdm.", actrpcivie=".$dadosContratos['actrpcivie'].", ";
            ...
            $sql .=" nctrpcoerl='".$dadosContratos['nctrpcoerl']."', nctrpcufrl='".$dadosContratos['nctrpcufrl']."',".$nctrpccdrl.$nctrpcedrl;
            $sql .= $nctrpcnarl." cctrpcecrl='".$dadosContratos['cctrpcecrl']."',".$nctrpcprrl.$nctrpcmlrl." ";
            ...
            $sql .=" nctrpcmlgt='".$dadosContratos['nctrpcmlgt']."', ectrpctlgt='".$dadosContratos['ectrpctlgt']."' , dctrpcfivg='".$dadosContratos['dctrpcfivg']."', dctrpcinex='".$dadosContratos['dctrpcinex']."',   ";
            $sql .= !empty($dadosContratos['csolcosequ']) ? " csolcosequ='".$dadosContratos['csolcosequ']."',  ": "csolcosequ=null, ";
            $sql .="corglicodi=".$dadosContratos['corglicodi'].", ectrpcraza='".$dadosContratos['ectrpcraza']."', ";
            $sql .= $ctpcomcodi.$aforcrsequ.$aforcrseq1.$cctrpcccep.$ectrpclogr.$actrpcnuen.$ectrpccomp.$ectrpcbair.$nctrpccida.$cctrpcesta.$ectrpctlct;
            $sql .= $actrpcnuap.$actrpcnuad.$valGlobalAdApost.$valExecutado.$vctrpcvlor." cpnccpcodi = ".$dadosContratos['cpnccpcodi'].", ";
            $sql .=" tctrpculat='".$dadosContratos['tctrpculat']."', dctrpcfiex='".$dadosContratos['dctrpcfiex']."', adocpcnupa = ".$dadosContratos['adocpcnupa'].", adocpcvapa = ".$dadosContratos['adocpcvapa'].", ectrpnuf2 = '".$dadosContratos['ectrpnuf2']."'  where cdocpcsequ = ".$dadosContratos['cdocpcsequ'];
        ...
        public function GetGestorAlteradoApostilamento($codDoc){
            ...
            //BUSCA TODOS FISCAIS POR APOSTILAMENTO
            public function getFiscalApostilamento($codDoc){
            $sql = " SELECT fisc.nfiscdtipo as tipofiscal, fisc.nfiscdnmfs as fiscalnome, fisc.efiscdmtfs as fiscalmatricula, fisc.cfiscdcpff as fiscalcpf, 
            fisc.nfiscdmlfs as fiscalemail, fisc.efiscdtlfs as fiscaltel, docfi.cdocpcsequ as docsequ, docfi.fdocfisitu as docsituacao, 
            fisc.nfiscdencp as ent, fisc.efiscdrgic as registro 
            FROM sfpc.tbdocumentofiscalsfpc AS docfi 
            INNER JOIN sfpc.tbfiscaldocumento AS fisc ON (fisc.cfiscdcpff = docfi.cfiscdcpff)  
            INNER JOIN sfpc.tbdocumentosfpc doc on docfi.cdocpcsequ = doc.cdocpcsequ 
            WHERE docfi.cdocpcsequ = " .$codDoc. " and doc.cfasedsequ = 6 and doc.csitdcsequ = 1; ";
            $resultado = executarPGSQL($sql);
            $dadosRetorno = array();
            while($resultado->fetchInto($retorno, DB_FETCHMODE_OBJECT)){
                $dadosRetorno[] = $retorno;
        ...
        public function CancelaContrato($codDoc,$codObs){
            $sql  = "UPDATE sfpc.tbdocumentosfpc SET csitdcsequ=2, cfasedsequ=1";
            $sql .= " where cdocpcsequ =".$codDoc;
             $resultado = executarSQL($this->conexaoDb, $sql);
             if(!empty($resultado)){
                    $sql  = "UPDATE sfpc.tbcontratosfpc SET ectrpcobse ='".$codObs."'";
                    $sql .= " where cdocpcsequ =".$codDoc;
                    $resultado = executarSQL($this->conexaoDb, $sql);
                 return $resultado;
             }else{
        ...
        public function DesfCancelaContrato($codDoc){
            $sql  = "UPDATE sfpc.tbdocumentosfpc SET csitdcsequ=1, cfasedsequ=1";
            $sql .= " where cdocpcsequ =".$codDoc;
             $resultado = executarSQL($this->conexaoDb, $sql);
             if(!empty($resultado)){
                $sql  = "UPDATE sfpc.tbcontratosfpc SET ectrpcobse =''";
                $sql .= " where cdocpcsequ =".$codDoc;
                $resultado = executarSQL($this->conexaoDb, $sql);
                 return $resultado;
             }else{
        ...
        public function CalculaDataFinalDeExecucao ($dataIniExe,$diaOrMes,$prazo){
            ...
            if($diaOrMes == "M"){
                return date("d/m/Y",mktime(0,0,0,intval($resultData[1])+intval($prazo),intval($resultData[0]),intval($resultData[2])));
            }else if($diaOrMes == "D"){
                return date("d/m/Y",mktime(0,0,0,$resultData[1],intval($resultData[0])+intval($prazo),intval($resultData[2])));
            }
        }

        public function VerificaSeExisteContratoComEssaSCC($NumSCC,$FornecedorScc,$OrigemScc,$citelpnuml,$idContrato){
                ...
                if(!empty($retorno)){
                    if($OrigemScc == "LICITACAO"){
                        ...
                        $sqlcitelpnuml .= "'$citelpnuml[$i]'";
                                    ...
                                    where c.cdocpcsequ <> ".$idContrato." and  c.aforcrseq1 = $FornecedorScc and c.csolcosequ = $NumSCC
                                    and cmatepsequ in (select iteml.cmatepsequ from sfpc.tbsolicitacaocompra scc 
                                    ...
                                    $sqlItem .= " order by iteml.aitelporde)  
                                    or cservpsequ in (select iteml.cservpsequ 
                                                        from sfpc.tbsolicitacaocompra scc 
                                                        inner join sfpc.tbsolicitacaolicitacaoportal solic on solic.csolcosequ = scc.csolcosequ 
                                                        inner join sfpc.tbitemlicitacaoportal iteml on solic.clicpoproc = iteml.clicpoproc and solic.alicpoanop = iteml.alicpoanop 
                                                        and solic.cgrempcodi = iteml.cgrempcodi and solic.ccomlicodi = iteml.ccomlicodi 
                                                        where scc.csolcosequ = $NumSCC
                                                        and iteml.aforcrsequ = $FornecedorScc ";
                                if($citelpnuml[0] != 0){                        
                                    $sqlItem .= "and iteml.citelpnuml in ($sqlcitelpnuml) ";
                                    ...
                                    $sqlItem .= " order by iteml.aitelporde)
                                    and c.aforcrsequ = $FornecedorScc and c.csolcosequ = $NumSCC";
                            //   var_dump($sqlItem);
        ...
        */
        ...
        //Pega o perfil do usuaruio 
        public function GetPerfilUser($codUsuario){
            $sql  = " SELECT cperficodi as perfil
                      from sfpc.tbusuarioperfil
                      WHERE cusupocodi = $codUsuario";
