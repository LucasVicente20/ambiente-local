...
class ExportaODS{
	...
	private function adicionaDados($arrayDados){
		...
		$colNum = 0;
		//$arrayDados = str_replace("&", "&amp;", $arrayDados);
		foreach($arrayDados as $row) {
			...
			foreach($row as $field) {
				...
				else
				{
					...
					$escapedField = str_replace("<", "&lt;", $escapedField);
					$escapedField = str_replace(">", "&gt;", $escapedField);
					$escapedField = str_replace("M²", "Metro quadrado", $escapedField);
					$escapedField = str_replace("M³", "Metro cúbico", $escapedField);
					$escapedField = str_replace("ª", " a ", $escapedField);
					$escapedField = str_replace("º", " o ", $escapedField);
					//$escapedField = RetiraAcentos($escapedField);
					//$escapedField = removeSimbolos($escapedField);
							
					$escapedField = preg_replace('/[\x00-\x1F\x7F-\xFF]/', ' ', $escapedField);
