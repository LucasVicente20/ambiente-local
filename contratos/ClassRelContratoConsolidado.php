...
# -------------------------------------------------------------------------
# Portal da Compras
# Programa: ClassRelContratoConsolidado.php
# Autor:    Eliakim Ramos | João Madson
# Data:     11/12/2019
# -------------------------------------------------------------------------
    
    Class RelContratoConsolidado {
        public $conexaoDb;
        ...
        public function GetApostilamento($codDoc){
         ...
         // Classe herdada de medição que difere da acima
        public function getApostilamentoHM($codCont){
            $db = Conexao();
        ...
        public function UpdateContrato($dadosContratos){
            ...
            $valGlobalAdApost = !empty($dadosContratos['valGlobalAdApost'])?" vctrpceant='".$dadosContratos['valGlobalAdApost']."', ":" ";
            // $valExecutado = !empty($dadosContratos['valExecutado'])?" vctrpceant='".$dadosContratos['valExecutado']."', ":" ";
            ...
            $sql .= $actrpcnuap.$actrpcnuad.$valGlobalAdApost.$valExecutado.$vctrpcvlor;
            $sql .=" tctrpculat='".$dadosContratos['tctrpculat']."', dctrpcfiex='".$dadosContratos['dctrpcfiex']."'  where cdocpcsequ = ".$dadosContratos['cdocpcsequ'];
            $resultado = executarSQL($this->conexaoDb, $sql);
            if(!empty($resultado)){
                return $resultado;
            }else{
