...
# -------------------------------------------------------------------------
#Programa: ApostilamentoGeral.php
# Autor:    Marcello Albuquerque
# Data:     09/11/2021
# CR #255536
# -------------------------------------------------------------------------
    
    Class ApostilamentoGeral {
        public $conexaoDb;
        ...
        public function getDocumentosFicaisEFicalApost($codDoc){
            ...
            docfi.fdocfisitu as docsituacao, fisc.nfiscdencp as entidade, fisc.efiscdrgic as registro 
            FROM sfpc.tbfiscaldocumento AS fisc
            INNER JOIN sfpc.tbdocumentofiscalsfpc AS docfi  ON (fisc.cfiscdcpff = docfi.cfiscdcpff) 
            inner join sfpc.tbapostilamento as apost on (docfi.cdocpcsequ = apost.cdocpcsequ)
        ...
        public function insertsContratoAntigoIncluir($dadosSalvar){
           ...
           // var_dump($itens_servico);die;
            ...
            $sqlC .= "fctrpcserc, dctrpcpbdm, fctrpcobra, tctrpculat, ctpcomcodi, fctrpccons) ";
            $sqlC .= "values (";
            ...
            $sqlC .= "'".strtoupper($dadosSalvar['fctrpcserc'])."', ".$dctrpcpbdm.", '".strtoupper($dadosSalvar['fctrpcobra'])."', now(), ".$dadosSalvar['ctpcomcodi'].", '".$consorcio."')";  
             //var_dump($sqlC);die;
           $resC = $db->query($sqlC);
           if(empty($resC)){
            var_dump('erro 4');die;
        ...
        public function insertsContratoIncluir($dadosSalvar){
            ...
            $sqlC .= "fctrpcobra, tctrpculat, fctrpccons, ctpcomcodi) ";
            ...
            $sqlC .= "'".strtoupper($dadosSalvar['fctrpcobra'])."', now(), '".strtoupper($dadosSalvar['fctrpccons'])."', '".$dadosSalvar['ctpcomcodi']."')";  
            // print_r($sqlC);
            $resC = $db->query($sqlC);
            if(empty($resC)){
                return 4;
             ...
             // var_dump(count($dadosSalvar['fornecedor']));die;
             if(!empty($dadosSalvar['fornecedor'])){
                for($i=0; $i>count($dadosSalvar['fornecedor']); $i++){
                    $sqlf = " INSERT INTO sfpc.tbcontratofornecedor (cdocpcsequ, aforcrsequ, cusupocodi, tconfrulat) ";
