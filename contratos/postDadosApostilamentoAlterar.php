<?php
# -------------------------------------------------------------------------
#Programa: postDadosApostilamentoAlterar.php
...
switch($_POST['op']){
    ...
    case "alterarApostilamento":
        ...
        $dados_validar = array(
           // 'tipo_apostilamento' => $idtipoApostilamento[0]->ctpaposequ,
            'tipo_apostilamento' => $idtipoApostilamento,
        ...
        if($validando != false){
            ...
            $id_tipo_apostilamento = intval($idtipoApostilamento);
            $dadosMedicao = array(

                // ADICIONAR CODIGO DO FORNECEDOR
           ...
           if(intval($idtipoApostilamento[0]->ctpaposequ) == 2 || intval($idtipoApostilamento[0]->ctpaposequ) == 3){
                ...
                foreach($_SESSION['documento_anexo_incluir'] as $docAnex){
                    ...
                    if($docAnex['remover'] == "S" && !empty($seExisteDocumentoAnexo)){
                        ...
                        if(empty($seExisteDocumentoAnexo) && $docAnex['remover'] != "S"){
                            ...
                            if(empty($retorno)){
                                $response = array("status"=>false,"msm"=>"Error não foi possivel editar o apostilamento, tente novamente");
                                print(json_encode($response));
                                exit;
                            }
                        }
            ...
            if($dadosMedicao['situacao_apost'] == 6 && $idtipoApostilamento == 5) {
                $atualizaFornecedor = TRUE;
                $existeApostilamentoFornecedor = $objFuncaoContrato->ValidaOFornecedorUpdateContratoapostilamento($dadosMedicao['CDOCPCSEQ2']);
                if(strtotime($dataApostilamento) < strtotime($existeApostilamentoFornecedor->data_cadastro)){
                    $atualizaFornecedor = FALSE;
                    $cod_fornecedor = $existeApostilamentoFornecedor->aforcrsequ;
                }
                $existeAdtivoFornecedor = $objFuncaoContrato->ValidaOFornecedorUpdateContratoadtivo($dadosMedicao['CDOCPCSEQ2']);
                if(strtotime($dataApostilamento) < strtotime($existeAdtivoFornecedor->data_cadastro)){
                    $atualizaFornecedor = FALSE;
                    $cod_fornecedor = $existeAdtivoFornecedor->aforcrsequ;
                }
                if($atualizaFornecedor){
                    $fornecedorApostilamento = $objFuncaoContrato->GetFornecedorByCod($cod_fornecedor);
                    $dadosFornecedor = array(
                                            'aforcrsequ' => $fornecedorApostilamento->aforcrsequ,
                                            'ectrpcraza' => $fornecedorApostilamento->nforcrrazs,
                                            'cctrpcccep' => $fornecedorApostilamento->cctrpcccep,
                                            'ectrpclogr' => $fornecedorApostilamento->eforcrlogr,
                                            'actrpcnuen' => $fornecedorApostilamento->aforcrnume,
                                            'ectrpccomp' => $fornecedorApostilamento->eforcrcomp,
                                            'ectrpcbair' => $fornecedorApostilamento->eforcrbair,
                                            'nctrpccida' => $fornecedorApostilamento->nforcrcida,
                                            'cctrpcesta' => $fornecedorApostilamento->cforcresta,
                                            'ectrpctlct' => $fornecedorApostilamento->ectrpctlct,
                                            'cdocpcseq2' => $dadosMedicao['CDOCPCSEQ2']
                    );
                    $seAlterouContrato = $objFuncaoContrato->updateFornecedorContrato($dadosFornecedor);
                    if(!$seAlterouContrato){
                        $response = array("status" => false,"msm" => "Contrato não atualizado.");
                        print(json_encode($response));
                        exit;
                        break;
                    }
    ...
    case "IncluirAditivo":
        ...
        $dadosAditivo = array(
            ...
            'AFORCRSEQU' => $ObjMedicao->anti_injection($codFornecedor),
