...
require_once "ClassApostilamentoIncluirPesquisar.php";

$ObjContrato = new ContratoPesquisar();
$ObjApostilamentoIncluir = new ApostilamentoIncluirPesquisar();
$arrayTirar  = array('.',',','-','/');
...
switch($_POST['op']){
    ...
    case "OrgaoGestor":
        ...
        if(count($dados) > 1){
            // $option.= '<option value="'.implode(',',$arrayCode).'">Selecione um órgão...</option>';
    ...
    case "PesquisaContrato":
        ...
        $dadosTabela = $ObjApostilamentoIncluir->Pesquisar($ArrayDados);
        ...
        foreach($dadosOrgao as $infOrgao){
                foreach($dadosTabela as $informacoesTable){
                    if($informacoesTable->eorglidesc == $infOrgao->eorglidesc){
                        ...
                        $situacaoForne = $ObjContrato->GetSituacaoFornecedor($informacoesTable->aforcrsequ);
                        $TipoCompra    = $ObjContrato->GetTipoCompra($informacoesTable->ctpcomcodi);
                        $SituacaoContrato  = $ObjApostilamentoIncluir->GetSituacaoContrato($informacoesTable->cfasedsequ);
