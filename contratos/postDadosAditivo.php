...
switch($_POST['op']){
    case "exibe_ornecedor_session":
       ...
       if(!empty($_SESSION['dados_fornecedor_incluir'])){
         // var_dump($_SESSION['dados_fornecedor_incluir']);
                ...
                $fornecedorContrato = $_SESSION['dados_fornecedor_incluir']; // $ObjClassAditivoGeral->GetFornecedorContrato($_POST['idregistro']);
              ...
              //      var_dump($listaFornecedor);
                    ...
                    $ObjClassAditivoGeral->DesconectaBanco();
                    print_r($ObjJS);
                }else{
    ...
    case "Fornecerdor2":
        if($_POST['repassaSessao'] != 'true'){
            ...
            $Cpf  = "";
            if(!empty($_POST['cpf'])){
                $Cpf = $ObjClassAditivoGeral->anti_injection(str_replace($arrayTirar,'',$_POST['cpf']));
            }
            if(!empty($_POST['cnpj'])){
                $cnpj = $ObjClassAditivoGeral->anti_injection(str_replace($arrayTirar,'',$_POST['cnpj']));
            }

            $DadosFornecedor = $ObjClassAditivoGeral->GetFornecedor($Cpf,$cnpj);
            if(is_null($DadosFornecedor->aforcrsequ)){
                ...
                if(!empty($Cpf) || !empty($cnpj)){
                  ...
                  //  var_dump($Cnpj);
                    $cpfCnpjFornmascarado = !is_null($DadosFornecedor->aforcrccgc) ? $ObjClassAditivoGeral->MascarasCPFCNPJ($DadosFornecedor->aforcrccgc) : $ObjClassAditivoGeral->MascarasCPFCNPJ($DadosFornecedor->aforcrccpf);
                    ...
                    if($validaRepeticao == true){
                        ...
                        $_SESSION['dadosFornecedor'][] = (object) array(
                                                                            ...
                                                                            'aforcrccpf'=>!empty($DadosFornecedor->aforcrccpf) ? $ObjClassAditivoGeral->MascarasCPFCNPJ($DadosFornecedor->aforcrccpf):$DadosFornecedor->aforcrccpf, 
                                                                            'aforcrccgc'=>!empty($DadosFornecedor->aforcrccgc) ? $ObjClassAditivoGeral->MascarasCPFCNPJ($DadosFornecedor->aforcrccgc):$DadosFornecedor->aforcrccgc,               
    ...
    case "ExibeFornecedorExtra":
        if(!empty($_POST['idregistro'])){
            if(empty($_SESSION['dadosFornecedor'])){
                $fornecedorContrato = $ObjClassAditivoGeral->GetFornecedorContrato($_POST['idregistro']);
                if(!empty($fornecedorContrato)){
                    foreach($fornecedorContrato as $fonec){
                            $listaFornecedor[] = (object) array(
                                                                'aforcrsequ'=>$fonec->aforcrsequ,
                                                                ...
                                                                'eforcrcomp'=>$fonec->eforcrcomp,
                                                                'eforcrbair'=>$fornec->eforcrbair,
                                                                'aforcrccpf'=>$ObjClassAditivoGeral->MascarasCPFCNPJ($fonec->aforcrccpf),
                                                                'aforcrccgc'=>$ObjClassAditivoGeral->MascarasCPFCNPJ($fonec->aforcrccgc),
                                                                'cforcresta'=>$fonec->cforcresta,
                                                                'remover'=>'N'
                                                            );
                    }
    ...
    case "Fiscal":
        ...
        if(!empty($_POST['cpf'])){
            $Cpf = $ObjClassAditivoGeral->anti_injection(str_replace($arrayTirar,'',$_POST['cpf']));
            if(!$ObjClassAditivoGeral->validaCPF($Cpf)){
        ...
        $tipo = $ObjClassAditivoGeral->anti_injection($_POST['tipo']);
        $DadosFiscal = $ObjClassAditivoGeral->GetFiscal($Cpf,$tipo);
        $response = array();
        if(empty($DadosFiscal)){
            ...
            // $DadosFiscal->cfiscdcpff = $ObjClassAditivoGeral->MascarasCPFCNPJ($DadosFiscal->cfiscdcpff);
            
            for($i = 0; $i < count($DadosFiscal); $i++){
                $DadosFiscal[$i]->cpfformatado = $ObjClassAditivoGeral->MascarasCPFCNPJ($DadosFiscal[$i]->cfiscdcpff);
            }
    ...
    case "SelecFiscal":
          ...
          if(!empty($_POST['cpf'])){
            ...
            $f=$ObjClassAditivoGeral->GetFiscal($dados['cfiscdcpff']);
            $_SESSION['fiscal_selecionado_incluir'][] = (object)  array(
                                                    ...
                                                    'fiscalcpf'       => strtoupper($ObjClassAditivoGeral->MascarasCPFCNPJ($f[0]->cfiscdcpff)),
    ...
    case "RemoveFiscal":
        ...
        // var_dump($_SESSION['fiscal_selecionado_incluir']);exit;
        ...
        if(!empty($sessaoFiscal)){
            foreach($sessaoFiscal as $f){
                ...
                if($MarcadorSeparado[0] == $cpflimpo[0]){
                    ...
                    array_splice($sessaoFiscal, $i, 1); // Remove os elementos (Array a ser alterado, Índice a ser removido, quantidade de itens a remover)
                }
                $i++;
            }
        }else{
    ...
    case "ModalAlterarFiscal":
        if(!empty($_POST['marcador'])){
            ...
            $html .= '<input type="text" name="cpffiscal" id="cpffiscal" size="11" value="'.$ObjClassAditivoGeral->MascarasCPFCNPJ($f[0]->cfiscdcpff).'" readonly required> </td> </tr>';
    ...
    break;
    case "alterarFiscal":
        $cpf      = trim($ObjClassAditivoGeral->anti_injection(str_replace($arrayTirar,'',$_POST['cpffiscal'])));
        $nome     = trim($ObjClassAditivoGeral->anti_injection($_POST['nomefiscal']));
        $email    = trim($ObjClassAditivoGeral->anti_injection($_POST['emailfiscal']));
        $telefone = trim($ObjClassAditivoGeral->anti_injection($_POST['telfiscal']));
        if(!empty($cpf)){
            $evalido = $ObjClassAditivoGeral->validaCPF($cpf);
            if(!empty($existe)){
        ...
        $dadosFiscal = array(
                            'cfiscdcpff'=> $cpf,
                            'nfiscdnmfs'=> $nome,
                            'nfiscdtipo'=> $ObjClassAditivoGeral->anti_injection($_POST['tipofiscalr']),
                            'nfiscdencp'=> $ObjClassAditivoGeral->anti_injection($_POST['entidadefiscal']),
                            'efiscdrgic'=> $ObjClassAditivoGeral->anti_injection($_POST['RegInsfiscal']),
                            'nfiscdmlfs'=> $email,
                            'efiscdtlfs'=> $telefone,
                            'cusupocodi'=> $ObjClassAditivoGeral->anti_injection($_SESSION['_cusupocodi_']),
                            );
        $retorno = $ObjClassAditivoGeral->UpdateFiscal($dadosFiscal);
        if(!empty($retorno)){
            $i =0;
            $f=$ObjClassAditivoGeral->GetFiscal( $cpf);
    ...
    case "excluirFiscal": 
        if(!empty($_POST['cpf'])){
            $cpf = $ObjClassAditivoGeral->anti_injection($_POST['cpf']);
            $tipo = $ObjClassAditivoGeral->anti_injection($_POST['tipo']);
            $temAlgumContrato = $ObjClassAditivoGeral->VerificaSeFiscalEstaEmAlgumDocumento($cpf);
            if(!$temAlgumContrato){
                    $retorno = $ObjClassAditivoGeral->DeletaFiscal($cpf);
                    if($retorno){
                        $dadosFiscal = $ObjClassAditivoGeral->GetFiscal(null, $tipo);
    ...
    case "PesquisaModalScc":
                ...
                if(!empty($_POST['dataIni']) || !empty($_POST['dataFim'])){
                    if(!empty($_POST['dataIni'])){
                        $dataIni = $ObjClassAditivoGeral->date_transform($_POST['dataIni']);
                        $dataIni = "'".$dataIni."'";
                    }
                    if(!empty($_POST['dataFim'])){
                        $dataFim = $ObjClassAditivoGeral->date_transform($_POST['dataFim']);
                        $dataFim = "'".$dataFim."'";
                    }
                }else{
                ...
                $tabelaDados = $ObjClassAditivoGeral->PesquisarSCC($ArrayDados);
                // $dadosTabela = $ObjClassAditivoGeral->PesquisarSCC($ArrayDados);
                $auxDadosTabela = array();
                for($d=0;$d<count($tabelaDados);$d++){
                    $origemD = (intval($tabelaDados[$d]->ctpcomcodi) == 2) ? "LICITACAO" : "OUTRA";
                    $validaScc = $ObjClassAditivoGeral->VerificaSeExisteContratoComEssaSCC($tabelaDados[$d]->csolcosequ, $tabelaDados[$d]->aforcrsequ, $origemD, $tabelaDados[$d]->citelpnuml);
                    if($validaScc == false){
                ...
                $tableHtml  .="<TBody>";
                        ...
                        foreach($dadosTabela as $informacoesTable){ 
                            ...
                            if($informacoesTable->multLote == true){
                                $TipoCompra    = $ObjClassAditivoGeral->GetTipoCompra($informacoesTable->ctpcomcodi);
                                ...
                                $tableHtml     .='<td> <input type="hidden" name="cpfselec-'.$informacoesTable->csolcosequ.'" value="'.$ObjClassAditivoGeral->MascarasCPFCNPJ($cpfCNPJ).'" />'.$ObjClassAditivoGeral->MascarasCPFCNPJ($cpfCNPJ).'</td>';
                                $tableHtml     .="</tr>";
    ...
    case "modalSccPesquisa":
        $origim = $ObjClassAditivoGeral->ListTipoCompra();
        $html  = '<div class="modal-title textonormal" >';
        ...
        $html .= '<tr> <td colspan="3" style="border:none;"> <div id="selectDivModal"> </div> </td> </tr>';
        $html .= '</table> </form> </div>';
        print_r($html);
    break;
    ...
    case "insertFiscal":
        ...
        $cpf      = trim($ObjClassAditivoGeral->anti_injection(str_replace($arrayTirar,'',$_POST['cpffiscal'])));
        $nome     = strtoupper(trim($ObjClassAditivoGeral->anti_injection($_POST['nomefiscal'])));
        $email    = trim($ObjClassAditivoGeral->anti_injection($_POST['emailfiscal']));
        $telefone = strtoupper(trim($ObjClassAditivoGeral->anti_injection($_POST['telfiscal'])));
        if(!empty($cpf)){
            $existe  = $ObjClassAditivoGeral->GetFiscal($cpf);
            $evalido = $ObjClassAditivoGeral->validaCPF($cpf);
            if(!empty($existe)){
                print_r(json_encode(array('msm'=>'Fiscal já cadastrado. Informe outro','Sucess'=>false)));
                exit;
        ...
        $dadosFiscal = array(
                            ...
                            'efiscdmtfs'=> $ObjClassAditivoGeral->anti_injection($_POST['matfiscal']),
                            'nfiscdtipo'=> strtoupper($ObjClassAditivoGeral->anti_injection($_POST['tipofiscalr'])),
                            'nfiscdencp'=> strtoupper($ObjClassAditivoGeral->anti_injection($_POST['entidadefiscal'])),
                            'efiscdrgic'=> strtoupper($ObjClassAditivoGeral->anti_injection($_POST['RegInsfiscal'])),
                            'nfiscdmlfs'=> $email,
                            'efiscdtlfs'=> $telefone,
                            'cusupocodi'=> $ObjClassAditivoGeral->anti_injection($_SESSION['_cusupocodi_']),
                            );
        $retorno = $ObjClassAditivoGeral->insertFiscal($dadosFiscal);
        if(!empty($retorno)){
            $DadosFiscal = $ObjClassAditivoGeral->GetFiscal($cpf);
            $_SESSION['fiscal_selecionado_incluir'][] = (object)  array(
    ...
    case "IncluirContrato":
        //Dados do fornecedor estão vindo pela session $_SESSION['dadosSalvar']
            ...
            if(!empty($_POST['numcontrato'])){
                ...
                // $existeContrato = $ObjClassAditivoGeral->VerificaSeExisteContrato($_POST['numcontrato']);
                if($existeContrato){
            ...
            if(!empty($dadosSalvar['numScc'])){
                ...
                $existeContrato = $ObjClassAditivoGeral->VerificaSeExisteContratoComEssaSCC($numeroSCC,$fornecedorScc,$origimSCC,$dadosSalvar['citelpnuml']);
            ...
            if($_POST["fieldConsorcio"] == "SIM"){
                $dadosSalvar['fctrpccons'] = $ObjClassAditivoGeral->corrigeString($_POST["fieldConsorcio"]);
                if(!empty($_SESSION['dadosFornecedor'])){
            ...
            if(!empty($_POST['fieldContinuo']) or !is_null($_POST['fieldContinuo'])){
                $dadosSalvar['fctrpcserc'] = $ObjClassAditivoGeral->corrigeString($_POST['fieldContinuo']);
            }else{
                $html .='Contínuo, ';
            }
           
            if(!empty($_POST["obra"])){
                $dadosSalvar['fctrpcobra'] = $ObjClassAditivoGeral->corrigeString($_POST["obra"]);
                if(!empty($_POST["cmb_regimeExecucaoModoFornecimento1"])){
                    $dadosSalvar['ectrpcremf'] = $ObjClassAditivoGeral->corrigeString($_POST["cmb_regimeExecucaoModoFornecimento1"]);
                }else{
            ...
            $nomeRep = $ObjClassAditivoGeral->corrigeString($_POST["repNome"]);     
            if(!empty($nomeRep)){
            ...
            if(!empty($_POST["repCPF"])){
               ...
               if(!$ObjClassAditivoGeral->validaCPF($dadosSalvar['ectrpccpfr'])){
            ...
            //dados gestor   
            $nomeGestor = $ObjClassAditivoGeral->corrigeString($_POST["gestorNome"]);                    
            if(!empty($nomeGestor)){
            ...
            if(!empty($_POST["gestorCPF"])){
                ...
                if(!$ObjClassAditivoGeral->validaCPF($dadosSalvar['nctrpccpfg'])){
            ...
            $emailGestor = $ObjClassAditivoGeral->anti_injection($_POST["gestorEmail"]);
            if(!empty($emailGestor)){
                $dadosSalvar['nctrpcmlgt'] = $emailGestor;
            }else{
                $html .= 'E-mail do gestor, ';
            }
            $matriculaGestor = $ObjClassAditivoGeral->corrigeString($_POST["gestorMatricula"]);
            if(!empty($matriculaGestor)){
                $dadosSalvar['nctrpcmtgt'] = $matriculaGestor;
            }else{
                $html .= 'Matrícula do gestor, ';
            }
            $telefoneGestor = $ObjClassAditivoGeral->corrigeString($_POST["gestorTelefone"]);
            ...
            if($_POST["vigenciaDataInicio"] != "" && $_POST["vigenciaDataTermino"] != ""){
                ...
                if($checaVigIni == true && $checaVigTerm == true){
                    if($_POST["execucaoDataInicio"] != "" && $_POST["execucaoDataTermino"] != ""){
                       if($vigTerm < $vigIni){
                            ...
                            if($_POST["execucaoDataInicio"] == "" && $_POST["execucaoDataTermino"] == ""){
                                $htmlD .= 'Data início de execução, Data de término de execução, ';
                            }
                        }
                    }elseif($_POST["execucaoDataTermino"] == ""){
                        $htmlD .= 'Data de término de execução';
                    }elseif($_POST["execucaoDataInicio"] == ""){
                    ...
                    if($_POST["execucaoDataInicio"] == "" && $_POST["execucaoDataTermino"] == ""){
                        $htmlD .= 'Data de início de execução, Data de término de execução, ';
                    }elseif($_POST["execucaoDataTermino"] == ""){
                        $htmlD .= 'Data de término de execução';
                    }elseif($_POST["execucaoDataInicio"] == ""){
                        $htmlD .= 'Data de início de execução ';
                ...
                if($_POST["vigenciaDataInicio"] == "" && $_POST["vigenciaDataTermino"] == ""){
                    $htmlD .= 'Data de inicio de vigência, Data de término de vigência, ';
                }elseif($_POST["vigenciaDataInicio"] == ""){
                    $htmlD .= 'Data de inicio de vigência, ';
                }elseif($_POST["vigenciaDataTermino"] == ""){
            ...
            // if(!empty($_POST["dataPublicacaoDom"])){
                ...
                // $dadosSalvar['dctrpcpbdm'] = $ObjClassAditivoGeral->date_transform($_POST["dataPublicacaoDom"]);
            // }else{ $dadosSalvar['dctrpcpbdm'] = 'null' ;}
            $dadosSalvar['dctrpcpbdm'] =  $ObjClassAditivoGeral->date_transform($_POST["dataPublicacaoDom"]);
            $dadosSalvar['ctipgasequ'] =  strtoupper($ObjClassAditivoGeral->corrigeString($_POST["comboGarantia"]) )  ;
            $dadosSalvar['nctrpccgrl'] =  strtoupper($ObjClassAditivoGeral->corrigeString($_POST["repCargo"]) );
            $dadosSalvar['ectrpcidrl'] = strtoupper($_POST["repRG"]);
            $dadosSalvar['nctrpcoerl'] =  strtoupper($ObjClassAditivoGeral->corrigeString($_POST["repRgOrgao"]));
            $dadosSalvar['nctrpcufrl'] =  strtoupper($ObjClassAditivoGeral->corrigeString($_POST["repRgUF"]));
            $dadosSalvar['nctrpccdrl'] =  strtoupper($ObjClassAditivoGeral->corrigeString($_POST["repCidade"])) ;
            $dadosSalvar['nctrpcedrl'] = strtoupper( $ObjClassAditivoGeral->corrigeString($_POST["repEstado"]) );
            $dadosSalvar['nctrpcnarl'] =  strtoupper($ObjClassAditivoGeral->corrigeString($_POST["repNacionalidade"]));
            $dadosSalvar['cctrpcecrl'] =  strtoupper($ObjClassAditivoGeral->corrigeString($_POST["repEstCiv"]));
            $dadosSalvar['nctrpcprrl'] = strtoupper( $ObjClassAditivoGeral->corrigeString($_POST["repProfissao"]));
            $dadosSalvar['nctrpcmlrl'] =  $ObjClassAditivoGeral->anti_injection($_POST["repEmail"]);
            ...
            if($html != "" && strlen($html) >1){
                ...
                if($dadosSalvar['origemScc'] == "LICITACAO"){
                    ...
                    $valoresItem = $ObjClassAditivoGeral->selectsContratoIncluir($dadosSalvar);
                }else{
               ...
               foreach($valoresItem as $item){
                ...
                if(empty($dadosSalvar['cdocpcsequ'])){
                    ...
                    $dadosSalvar['cdocpcsequ'] = $ObjClassAditivoGeral->selectsContratoIncluir($dadosSalvar);
                    if(!empty($dadosSalvar['cdocpcsequ'])){
                        $itemContrato = $ObjClassAditivoGeral->insertsContratoIncluir($dadosSalvar);
                        if($itemContrato != true && ($itemContrato == 1 || $itemContrato ==2 || $itemContrato == 3 || $itemContrato == 4)){
                            $ObjClassAditivoGeral->deletsContratoIncluir($itemContrato, $dadosSalvar['cdocpcsequ']);
                ...
                if($itemContrato != true){
                    ...
                    if(!empty($_SESSION['dadosFornecedor'])){  
                        foreach($_SESSION['dadosFornecedor'] as $FornecedorSession){
                            $seExisteFornecedorContratoCadastrado = $ObjClassAditivoGeral->VerificarSeExisteFornecedorContrato($dadosSalvar['cdocpcsequ'],$FornecedorSession->aforcrsequ);
                            if($FornecedorSession->remover == 'N'){
                                if(empty($seExisteFornecedorContratoCadastrado->aforcrsequ)){
                                    ...
                                    $retornoFC = $ObjClassAditivoGeral->InsertFornecedorContrato($dadosFornecedorContrato);
                                    if(empty($retornoFC)){
                                ...
                                $retornoFCDelete = $ObjClassAditivoGeral->DeleteFornecedorContrato($dadosSalvar['cdocpcsequ'],$FornecedorSession->aforcrsequ);
                            }
                        }
                        unset($_SESSION['dadosFornecedor']);
                    }
                        ...
                        foreach($_SESSION['documento_anexo_incluir'] as $docAnex){
                            if($docAnex)
                                if($docAnex['remover'] != "S"){
                                    ...
                                    $retorno = $ObjClassAditivoGeral->InsertDocumentosAnexos($dadosDocAnex);
                                    if(empty($retorno)){
      ...
      //  var_dump($_SESSION['dados_fornecedor_incluir']);
       ...
       // var_dump( $dadosSalvar['itens_servico']);die;
        ...
        if($opcao_cpf == 1){
            ...
            $cod_fornecedor = $ObjClassAditivoGeral->GetFornecedor('', $valor_cpf_cnpj);
        ...
        $valor_original = $ObjClassAditivoGeral->floatvalue($_POST['valor_original']);
        $valor_global = $ObjClassAditivoGeral->floatvalue($_POST['valor_global']);
        
        $valor_executado_acumulado = $ObjClassAditivoGeral->floatvalue($_POST['valor_executado_acumulado']);
       ...
       // var_dump($dadosSalvar['aforcrsequ']);
        $dadosSalvar['aforcrsequ'] = $ObjClassAditivoGeral->corrigeString($cod_fornecedor->aforcrsequ);
        $dadosSalvar['ctpcomcodi'] = $ObjClassAditivoGeral->corrigeString($origem);
        $dadosSalvar['cctrpciden'] = $ObjClassAditivoGeral->corrigeString($opcao_cpf);
        $dadosSalvar['ectrpccpfr'] = $ObjClassAditivoGeral->corrigeString($opcao_informada);
        $dadosSalvar['fctrpcobra'] = $ObjClassAditivoGeral->corrigeString($obra);
        $dadosSalvar['fctrpccons'] = $ObjClassAditivoGeral->corrigeString($opcao_consorcio);
        $dadosSalvar['fctrpcserc'] = $ObjClassAditivoGeral->corrigeString($opcao_continuo);
        
        $dadosSalvar['fctrpcremf'] = $ObjClassAditivoGeral->corrigeString($regimeExecucaoModoFornecimento);
        $dadosSalvar['cctrpcopex'] = $ObjClassAditivoGeral->corrigeString($opcaoExecucaoContrato);
        $dadosSalvar['actrpcpzec'] = $ObjClassAditivoGeral->corrigeString($prazo);
        $dadosSalvar['dctrpcpbdm'] = $ObjClassAditivoGeral->corrigeString($dataPublicacaoDom);
        $dadosSalvar['dctrpcinvg'] = $ObjClassAditivoGeral->corrigeString($vigenciaDataInicio);
        $dadosSalvar['dctrpcfivg'] = $ObjClassAditivoGeral->corrigeString($vigenciaDataTermino);
        $dadosSalvar['dctrpcinex'] = $ObjClassAditivoGeral->corrigeString($execucaoDataInicio);
        $dadosSalvar['dctrpcfiex'] = $ObjClassAditivoGeral->corrigeString($execucaoDataTermino);
        
        $dadosSalvar['vctrpcvlor'] = $ObjClassAditivoGeral->anti_injection($valor_original);
        $dadosSalvar['vctrpcsean'] = $ObjClassAditivoGeral->anti_injection($valor_global);
        $dadosSalvar['vctrpceant'] = $ObjClassAditivoGeral->anti_injection($valor_executado_acumulado);
        $dadosSalvar['actrpcnuad'] = $ObjClassAditivoGeral->corrigeString($numero_ultimo_aditivo);
        $dadosSalvar['actrpcnuap'] = $ObjClassAditivoGeral->corrigeString($numero_ultimo_apostilamento);
        $dadosSalvar['ctipgasequ'] = $ObjClassAditivoGeral->corrigeString($comboGarantia);

        $dadosSalvar['nctrpcnmgt'] = $ObjClassAditivoGeral->corrigeString($gestorNome);
        $dadosSalvar['nctrpcmtgt'] = $ObjClassAditivoGeral->corrigeString($gestorMatricula);
        $dadosSalvar['nctrpccpfg'] = $ObjClassAditivoGeral->corrigeString($gestorCPF);
        $dadosSalvar['nctrpcmlgt'] = $ObjClassAditivoGeral->anti_injection($gestorEmail);
        $dadosSalvar['ectrpctlgt'] = $ObjClassAditivoGeral->corrigeString($gestorTelefone);

        $dadosSalvar['ectrpccpfr'] = $ObjClassAditivoGeral->corrigeString($repCPF);
        $dadosSalvar['nctrpccgrl'] = $ObjClassAditivoGeral->corrigeString($repCargo);
        $dadosSalvar['ectrpcidrl'] = $ObjClassAditivoGeral->corrigeString($repRG);
        $dadosSalvar['nctrpcoerl'] = $ObjClassAditivoGeral->corrigeString($repRgOrgao);
        $dadosSalvar['nctrpcufrl'] = $ObjClassAditivoGeral->corrigeString($repRgUF);
        $dadosSalvar['nctrpccdrl'] = $ObjClassAditivoGeral->corrigeString($repCidade);
        $dadosSalvar['nctrpcedrl'] = $ObjClassAditivoGeral->corrigeString($repEstado);
        $dadosSalvar['nctrpcnarl'] = $ObjClassAditivoGeral->corrigeString($repNacionalidade);
        $dadosSalvar['cctrpcecrl'] = $ObjClassAditivoGeral->corrigeString($repEstCiv);
        $dadosSalvar['nctrpcprrl'] = $ObjClassAditivoGeral->corrigeString($repProfissao);
        $dadosSalvar['nctrpcmlrl'] = $ObjClassAditivoGeral->anti_injection($repEmail);
        $dadosSalvar['ectrpctlrl'] = $ObjClassAditivoGeral->corrigeString($repTelefone);
        
        $dadosSalvar['corglicodi'] = $orgLicitante;
        $dadosSalvar['ectrpcobje'] = $ObjClassAditivoGeral->corrigeString($objetoDesc);
        ...
        $dadosSalvar['nctrpccida'] = $_SESSION['dadosSalvar']['nctrpccida'];
            ...
            if($_POST["fieldConsorcio"] == "SIM"){
                $dadosSalvar['fctrpccons'] = $ObjClassAditivoGeral->corrigeString($_POST["fieldConsorcio"]);
                if(!empty($_SESSION['dados_fornecedor_incluir'])){
                    $dadosSalvar['fornecedor'] = $_SESSION['dados_fornecedor_incluir'];
                    //var_dump($dadosSalvar['fornecedor']);die; 
                }
            }elseif($_POST["fieldConsorcio"] == "NAO"){
                $dadosSalvar['fctrpccons'] = $ObjClassAditivoGeral->corrigeString($_POST["fieldConsorcio"]);
            }else{
            ...
            */
            ...
            if($_POST["vigenciaDataInicio"] != "" && $_POST["vigenciaDataTermino"] != ""){
                ...
                }elseif(empty($_POST["vigenciaDataTermino"])){
            ...
            if($html != "" && strlen($html) >1){
               ...
               // $dadosSalvar['vctrpcvlor'] = $totalItens;
                ...
                if(empty($dadosSalvar['cdocpcsequ'])){
                    ...
                    if(!empty($dadosSalvar['cdocpcsequ'])){
                        $itemContrato = $ObjClassAditivoGeral->insertsContratoAntigoIncluir($dadosSalvar);
    ...
    case "IncluirMedicao":
        //Dados do fornecedor estão vindo pela session $_SESSION['dadosSalvar']
            ...
            $dadosMedicao = array(
                ...
                'dmedcoinic'=> $ObjClassAditivoGeral->date_transform($ObjMedicao->anti_injection($data_inicio)),
                'dmedcofinl'=> $ObjClassAditivoGeral->date_transform($ObjMedicao->anti_injection($data_termino)),
          ...
          //  $html = 'INFORME: ';
            ...
            //-------------------------------------------------------------------------------------------------------------
                ...
                //         if($docAnex)
                //             $seExisteDocumentoAnexo = $ObjClassAditivoGeral->GetDocumentosAnexos($docAnex['sequdoc'],$docAnex['sequarquivo'],$docAnex['nome_arquivo']);
    ...
    case "VerificaSeTemNumeroContrato": 
        ...
        if(!empty($numeroContrato)){
            $existeContrato = $ObjClassAditivoGeral->VerificaSeExisteContrato($numeroContrato);
            if($existeContrato){
                 $response = array("status"=>false,"msm"=>"Número de contrato já existe.");
                 print(json_encode($response));
            }else{
    ...
    case "VerificaSeTemNumeroContratoComSCC": 
        ...
        if(!empty($numeroSCC)){
            $existeContrato = $ObjClassAditivoGeral->VerificaSeExisteContratoComEssaSCC($numeroSCC,$ItemScc,$origimSCC,$lotes);
