...
# -------------------------------------------------------------------------
# Portal da Compras
# Programa: ClassContratoPesquisar.php
...
#---------------------------------------------------------------------------
    ...
    Class ContratoPesquisar {
       ...
       /* public function GetOrgao($internet = null){
        ...
        public function GetOrgaoById($dados, $internet = null){
        ...
        public function cfasedsequ($codSitucao){
            if(!empty($codSitucao)){
                $sql = "select sitdoc.esitdcdesc from sfpc.tbsituacaodocumento as sitdoc where sitdoc.cfasedsequ =".$codSitucao;
                $resultado = executarSql($this->conexaoDb, $sql);
                $resultado->fetchInto($retorno, DB_FETCHMODE_OBJECT);
        ...
        public function Pesquisar($dados){
            //Limpar simbolos para abrangir a pesquisa pelo objeto CR260440 |Madson
            if(!empty($dados['objeto'])){
                $objUpper = strtoupper2($_POST['objeto']);
                $objSemSimbol = RetiraAcentos($objUpper);
            }
            ...
            if(!empty($dados['numerocontratoano'])){
                ...
                if(!empty($dados['objeto'])){
                   // $sql .=" AND con.ectrpcobje like '%".$dados['objeto']."%'"; | Madson Removido para adaptar a alteração CR260440
                    $sql .=" AND (con.ectrpcobje like '%".$objUpper."%' or con.ectrpcobje like '%".$objSemSimbol."%')";
                }
                ...
                if (!empty($dados['dataInicial']) && !empty($dados['dataFinal'])) {
                    $sql .= " AND con.dctrpccada BETWEEN '".$this->date_transform($dados['dataInicial'])."' AND '".$this->date_transform($dados['dataFinal'])."'";
                }          
                ...
                $sql .= " ORDER BY 
                SUBSTRING(con.ectrpcnumf, position('/' in con.ectrpcnumf) + 1, 4) ASC,
                SUBSTRING(con.ectrpcnumf, position('.' in con.ectrpcnumf) + 1, 4) ASC,
                SUBSTRING(con.ectrpcnumf, position('/' in con.ectrpcnumf) + 5) ASC";    
            ...
            }else if($dados['vigente'] == "vigente"){
                
                $db = conexao();

                $sql  = "select orlic.eorglidesc, con.cdocpcsequ, doc.cfasedsequ,
                doc.csitdcsequ as codsequsituacaodoc, con.ectrpcnumf, con.aforcrsequ,
                con.ectrpcobje, con.ctpcomcodi, forn.aforcrsequ,
                            CON.dctrpcinvg as datainivige, CON.dctrpcfivg as
                datafimvige, adt.daditifivg as fimvigenciaadt, CON.actrpcivie as
                numdiasentreinivigeneexec, CON.actrpcfvfe as
                numdiasentrefimvigeneexec,
                            forn.aforcrccgc, forn.aforcrccpf, forn.nforcrrazs,
                SCC.csolcocodi, SCC.asolcoanos, CC.ccenpocorg, CC.ccenpounid,
                            CASE WHEN SCC.ctpcomcodi IS NULL THEN con.ctpcomcodi WHEN
                SCC.ctpcomcodi IS NOT NULL THEN SCC.ctpcomcodi END AS ctpcomcodi
                            from sfpc.tbcontratosfpc as con inner join
                sfpc.tbdocumentosfpc as doc on ( con.cdocpcsequ=doc.cdocpcsequ )
                            inner join sfpc.tbfornecedorcredenciado forn on (
                con.aforcrsequ=forn.aforcrsequ )
                            left outer join SFPC.TBSOLICITACAOCOMPRA SCC on (
                con.csolcosequ = SCC.csolcosequ )
                            left outer join SFPC.tbcentrocustoportal CC on
                CC.ccenposequ = SCC.ccenposequ
                            left outer join sfpc.tborgaolicitante orlic on (
                con.corglicodi= orlic.corglicodi )
                            left outer join sfpc.tbaditivo adt on (con.cdocpcsequ =
                adt.cdocpcseq1 and adt.faditialpz = 'SIM') where 1=1 and
                            (CASE WHEN adt.cdocpcseq1 IS NULL THEN (con.dctrpcfivg >= now())
                            WHEN (select MAX(adt2.aaditinuad) from sfpc.tbaditivo adt2
                left join sfpc.tbdocumentosfpc as doc2
                            ...
                            WHEN adt.cdocpcseq1 IS NOT NULL AND adt.faditialpz = 'SIM'
                THEN (adt.daditifivg >= now()
                            and adt.aaditinuad = (select MAX(adt2.aaditinuad) from
                sfpc.tbaditivo adt2
                            left join sfpc.tbdocumentosfpc as doc2 on (
                adt2.cdocpcsequ = doc2.cdocpcsequ and adt2.faditialpz = 'SIM')
                            where adt2.cdocpcseq1 = con.cdocpcsequ and adt2.faditialpz
                = 'SIM' and doc2.cfasedsequ = 4 and doc2.ctidocsequ = 2))
                            END)";
        ...
        public function ListaSituacaoDoc(){
            $sql = "select sit.cfasedsequ, sit.csitdcsequ, sit.esitdcdesc from sfpc.tbsituacaodocumento as sit where sit.cfasedsequ in (1,2) and sit.csitdcsequ in (1,2)";
            $resultado = executarSQL($this->conexaoDb, $sql);
            $dadosPesquisa = array();
            while($resultado->fetchInto($retorno, DB_FETCHMODE_OBJECT)){
                $dadosPesquisa[] = $retorno;
