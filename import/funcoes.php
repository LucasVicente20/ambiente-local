...
function Seguranca()
{
    ...
    /** troca n hifens por apenas 1 em qualquer parte do texto em qualquer posicao da variavel */
	...
	# Verifica se sessão não possui usuário e não foi limpa
	// [ORIGINAL]
	/*
    if(
        (($_SESSION['_cgrempcodi_']=="")||($_SESSION['_eusupologi_']==""))&&
    ...
    */
    ...
    if (isset($_SESSION['_eacepocami_']) && empty($_SESSION['_cgrempcodi_'])) {
    ...
    VerificarAmbiante();
    // if($_SESSION['_cgrempcodi_'] or $_SESSION['_eusupologi_']){ 
    $db = Conexao();
    $sql  = "SELECT EUSUPORESP, AUSUPOFONE ";
    $sql .= "  FROM SFPC.TBUSUARIOPORTAL ";
    $sql .= " WHERE CGREMPCODI = ? ";
    $sql .= "   AND EUSUPOLOGI = ? ";
    $result = $db->prepare($sql);
    $aux = array(
        $_SESSION['_cgrempcodi_'],
        $_SESSION['_eusupologi_']
    ) ;
    $result->execute($aux);
    if ( $db->errorInfo()[2] ) {
    // 		echo "
    // 			<html>
    // 				<body>
    // 					Login para o sistema falhou. Um email foi enviado para o Administrador.
    // 				</body>
    // 			</html>
    // 		";
    // 		EmailErroSQL("Checagem de acesso.", __FILE__, __LINE__, "Select de checagem de acesso falhou", $sql, $result);
    // 		NegarAcesso();
    } else {
    ...
    $VarGet = RetornaDados($_GET,0,'');
    $VarPost = RetornaDados($_POST,0,'');
    $MensagemEmail .= "GET......: \n".$VarGet."\n";
    $MensagemEmail .= "POST.....: \n".$VarPost."\n";
    ...
    if ( VarreDadosSeguranca($_GET)==1 or VarreDadosSeguranca($_POST)==1) {
            echo $Mensagem;
            EnviaEmail($GLOBALS["EMAIL_SUPORTE"],$NomeLocalTitulo." POSSIVEL TENTATIVA DE INVASÃO",$MensagemEmail,$GLOBALS["EMAIL_FROM"]);
            NegarAcesso();
    }

    /* Define caminho válido */
    $CaminhoValido = $GLOBALS["DNS_SISTEMA"];           // Desenvolvimento
    //$CaminhoValido = "http://www.recife.pe.gov.br/pr/secfinancas/portalcompras/programas"; // Produção

    /* Identifica página a ser validada */
    $Pagina = "/".$GLOBALS["PASTA_SISTEMA"].str_replace( $CaminhoValido, "", $_SERVER["SCRIPT_URI"] );
    //$Pagina = str_replace( $CaminhoValido, "", $_SERVER["REQUEST_URI"] );

    /*$teste="[?val=".$Pagina;
    $teste.="&val2=".$CaminhoValido;
    $teste.="&val3=".$_SERVER["REQUEST_URI"];
    $teste.="&val4=".$_SERVER["SCRIPT_URI"]."]";
    echo $teste;*/
    //exit;
    /* Define página como falsa */
    $PaginaFalsa = 0;
