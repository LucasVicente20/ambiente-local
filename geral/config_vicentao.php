...
$scriptUrl  = !empty($_SERVER["SCRIPT_URL"]) ? $_SERVER["SCRIPT_URL"] : "";
...
if ($ServerName == 'cohab.recife' and $scriptUrlPartes[1] == 'portalcompras') {
    ...
    if (is_null($GLOBALS["CAMINHO_SISTEMA"])) {
        $GLOBALS["CAMINHO_SISTEMA"] = 'C:/xampp/htdocs/'; // caminho para este sistema
    }
    ...
    //Comentado por Eliakim Ramos Pois a definição da constante esta comentata
    ...
    if (! is_null($GLOBALS["NOME_DESENVOLVEDOR"])) {
        ...
        $GLOBALS["CAMINHO_SISTEMA"] = 'C:/xampp/htdocs/'; // caminho para este sistema
        $GLOBALS["LOCAL_SISTEMA"]   =  ( $_SERVER["HTTP_HOST"] === "appcompras.recife") ? CONST_NOMELOCAL_DESENVOLVIMENTO : CONST_NOMELOCAL_HOMOLOGACAO;
        $GLOBALS["PASTA_SISTEMA"]   = "portalcompras/";
        $array                      = explode($GLOBALS["PASTA_SISTEMA"], $_SERVER['PHP_SELF']);
        $GLOBALS["DNS_SISTEMA"]     = $array[0] . $GLOBALS["PASTA_SISTEMA"];
        $GLOBALS["DATASOURCE_LABEL_ORACLE"] = ''; //CONST_DATASOURCE_LABEL_ORACLE_HOMOLOGACAO; //Comentado por Eliakim Ramos Pois a definição da constante esta comentata
        $GLOBALS["BD_ORACLE_USUARIO"]       = "us_portal";
        $GLOBALS["BD_ORACLE_SENHA"]         = "portal";
        ...
        $GLOBALS["CAMINHO_SISTEMA"]       = $GLOBALS["CAMINHO_SISTEMA"] . $GLOBALS["PASTA_SISTEMA"];
        ...
        $GLOBALS["DNS_UPLOADS"]           = $GLOBALS["DNS_DOMINIO"]  . 'uploads/';
    ...
    if (! is_null($GLOBALS["NOME_DESENVOLVEDOR"])) {
        ...
        error_reporting(E_ALL ^ E_NOTICE);
        // Servidor sendo rodado em algum ambiente fora do padrão que é informado pelo configvars
        $GLOBALS["CAMINHO_SISTEMA"] = '/var/www/html/secfinancas/'; // caminho para este sistema
        $GLOBALS["LOCAL_SISTEMA"]   =  ( $_SERVER["HTTP_HOST"] === "appcompras.recife") ? CONST_NOMELOCAL_DESENVOLVIMENTO : CONST_NOMELOCAL_HOMOLOGACAO;
        $GLOBALS["PASTA_SISTEMA"]   = "portalcompras/";
        $array                      = explode($GLOBALS["PASTA_SISTEMA"], $_SERVER['PHP_SELF']);
        $GLOBALS["DNS_SISTEMA"]     = $array[0] . $GLOBALS["PASTA_SISTEMA"];
...
if (is_null($GLOBALS["CAMINHO_UPLOADS"]))
    $GLOBALS["CAMINHO_UPLOADS"] = '/var/www/html/secfinancas/portalcompras/pr/secfinancas/portalcompras/uploads/'; // Caminho para salvar arquivos de upload
if (is_null($GLOBALS["CAMINHO_LOGS"]))
    $GLOBALS["CAMINHO_LOGS"] = '/var/www/html/secfinancas/portalcompras/pr/secfinancas/portalcompras/uploads/'; // Caminho para salvar arquivos de log
...
if (is_null($GLOBALS["CAMINHO_EMAIL"]))
    $GLOBALS["CAMINHO_EMAIL"] = '/var/www/html/secfinancas/portalcompras/common/phpmailer/';
    // $GLOBALS["CAMINHO_PDF"] = '/var/www/html/secfinancas/portalcompras/common/fpdf/';
